function DSBC() {
  ...
  chart.data = (value) => {
      ...
      rawFile.onreadystatechange = function () {
        if (rawFile.readyState === 4) {
          if (rawFile.status === 200 || rawFile.status == 0) {
            ...
            if (fileName.indexOf("time") != -1) leftAxis = "次數";
            else if (fileName.indexOf("size") != -1) leftAxis = "下載量";
            else leftAxis = fileName;
    ...
    let sortData = (data, sortByKey) => {
      ...
      for (i in sortedData) console.debug(sortedData[i].name);
      console.debug("=============");
  ...
  function chart() {
      ...
      let initData = () => {
        const convertData = () => {
          ...
          dataKeys.forEach((key) => {
            let obj = data[key];
            let DBKeys = Object.getOwnPropertyNames(obj).filter(
              (key) => key != "columns" && key != "total"
            );
            obj.columns = DBKeys;
            ...
            DBKeys.forEach((DBkey) => {
              ...
              if (typeof obj[DBkey] == "object") {
                let yearKeys = Object.getOwnPropertyNames(obj[DBkey]).filter(
                  (key) => key != "columns"
                );
                ...
                if (key == "file_size") {
                  ...
                  yearKeys.forEach((yearKey) => {
                    ...
                    if (typeof obj[DBkey][yearKey] == "string")
                      obj[DBkey][yearKey] = split_and_convert(
                        obj[DBkey][yearKey],
                        defaultSizeUnit
                      );
                  });
          ...
          data.columns = dataKeys.filter((key) => {
            ...
            if (data[key].hasOwnProperty("total"))
              if (data[key].total == 0) boolean = false;
            return boolean;
          });
          // console.debug(data);
        };
        const getDataKey = () => {
          ...
          subjects = Array.from(
            new Set(
              [].concat(
                ...dataKeys.map((key) => [].concat(...data[key].columns))
              )
            )
          );
          ...
          categories = Array.from(
            new Set(
              [].concat(
                ...dataKeys.map((key) =>
                  [].concat(
                    ...data[key].columns.map((k) => data[key][k].columns)
                  )
                )
              )
            )
    ...
    function doubleChart() {
                ...
                <div class="row">
                    ...
                    <div id="displayOption" class="form-group col-lg-3 col-md-3 col-sm-6 d-flex flex-row align-items-start">
                        ...
                        <div class="btn-group btn-group-toggle col-7" role="group">
                            ...
                            <div class="dropdown-menu p-4" id="displayMenu" aria-labelledby="displaySelectButton">
                                <div class="row" id="displayDropDownMenu" style="width:250px;">
        ...
        let displayHTML = subjects.map(
          (key, i) =>
            `<div class="form-check col-6">
                <input class="form-check-input" type="checkbox" id="${key}Display" name="display" value="${key}" checked>
                <label class="form-check-label" for="${key}Display">${key}</label>
            </div>`
        );

        $("#displayDropDownMenu").append(displayHTML);
      ...
      let newDataObj;
      let subjectScale, leftScale, rightScale;
      ...
      function updateChart(trans = false) {
        ...
        function init() {
          ...
          if (categories.length >= 2) {
            ...
            let legend = legendGroup
              ...
              .attr(
                ...
                `translate(${
                  width -
                  margin.right -
                  categories.length * (rect_width + rect_interval)
                }, ${margin.top * 0.6})`
              );
            ...
            legend
              ...
              .attr(
                "transform",
                (d, i) => `translate(${i * (rect_width + rect_interval)}, 0)`
              )
              .call((g) => {
                g.append("rect")
        ...
        function render() {
          ...
          if (barOption.orientation) {
            ...
            rightRange = [
              leftRange[0] + barOption.interval,
              width - margin.right,
            ];
          }
          ...
          let updateAxis = () => {
            ...
            let removeAxis = (g) => g.selectAll(":not(.axisName)").remove();
            let makeSubjectAxis = (g) => {
                ...
                refreshing = (g) => {
                  ...
                  g.selectAll(".domain")
                    ...
                    .attr(
                      "transform",
                      (d, i) =>
                        `translate(${barOption.interval * 0.5 * (i * 2 - 1)},0)`
                    );
                  ...
                  g.select(".axisName").attr(
                    "transform",
                    `translate(0,${height - margin.bottom * 0.2})`
                  );
                };
              }
            ...
            let makeSeriesAxis = (g, isRight) => {
              ...
              if (barOption.orientation) {
                ...
                translate = {
                  0: [margin.left, 0],
                  1: [width - margin.right, 0],
                }[+isRight];
                refreshing = (g) => {
                  g.select(".axisName")
                    .attr(
                      "transform",
                      `rotate(${90 * sign}) translate(${[
                        (height / 2) * sign,
                        -margin.left * 0.9,
                      ]})`
                    )
              ...
              g.attr("transform", `translate(${translate})`)
                ...
                .call((g) => {
                  ...
                  (isRight ? rightAxisOption : leftAxisOption).logScale
                    ? axisFun.ticks(
                        Math.log10(
                          seriesScale.domain()[1] / seriesScale.domain()[0]
                        ) + 1,
                        formatPower
                      )
                    : axisFun.ticks(5);
                ...
                barGroup_collection.each(function (dataKey, i) {
                  ...
                  seriesGroup
                    ...
                    .property(
                      "value",
                      (d, index) =>
                        i * subjects.length * categories.length +
                        ...
                        if (barOption.orientation) {
                          ...
                          let barWidth =
                            subjectScale.bandwidth() / 2 > barOption.maxWidth
                              ? barOption.maxWidth
                              : subjectScale.bandwidth() / 2;
                          let transX = i
                            ? subjectScale.bandwidth() / 2 + barOption.interval
                            : subjectScale.bandwidth() / 2 -
                              barWidth -
                              barOption.interval;
          ...
          newDataObj = getNewData({
            leftAxisOption: {
              metric: chartContainerD3
                .select("input[name=leftAxisMetric]:checked")
                .property("value"),
              logScale: false,
            },
            rightAxisOption: {
              metric: chartContainerD3
                .select("input[name=rightAxisMetric]:checked")
                .property("value"),
              logScale: false,
            },
            displayArr: [...subjects],
      ...
      function getNewData(chartOption) {
        let leftAxisOption = chartOption.hasOwnProperty("leftAxisOption")
            ? chartOption.leftAxisOption
            : newDataObj.leftAxisOption,
          rightAxisOption = chartOption.hasOwnProperty("rightAxisOption")
            ? chartOption.rightAxisOption
            : newDataObj.rightAxisOption,
          ...
          showArr = chartOption.hasOwnProperty("showArr")
            ? chartOption.showArr
            : newDataObj.showArr,
          orientation = chartOption.hasOwnProperty("orientation")
            ? chartOption.orientation
            : newDataObj
        ...
        let getSeries = (key) => {
          ...
          const series = d3
            ...
            .value((subject, category) =>
              seriesData[subject] ? seriesData[subject][category] || 0 : 0
            )(
              //沒有值當0(bar heigth=0)
              subjects
            )
      ...
      function events(svg) {
        ...
        const subjectTickCollection = svg
          .select(".subjectAxis")
          .selectAll(".tick");
        ...
        let tooltipEvent = () => {
          ...
          function makeTotalTooltip(subjectIndex, seriesIndex) {
            let totalTooltipID =
              ...
              "-" +
              dataKeys[seriesIndex];
            let totalTooltip_exist =
              svg.select("#" + totalTooltipID).node() != null;
            // console.debug(totalTooltip_exist);
            //=== if totalTooltip exist then do nothing
            if (!totalTooltip_exist) {
              let barValue =
                seriesIndex * subjects.length * categories.length +
                subjectIndex +
                (categories.length - 1) * subjects.length;
              // console.debug(barNodes);
              let topRect = barNodes[barValue];
              //   console.debug(topRect);
              let rectMaxData = topRect.__data__[1];
              // console.debug(topRect.__data__  );
              let total = Number.isInteger(rectMaxData)
                ? rectMaxData
                : rectMaxData.toFixed(3);
              ...
              totalTooltip.call((totalTooltip) => {
                ...
                totalTooltip
                  ...
                  .attr(
                    "points",
                    `${rect_width * 0.4}, ${rect_height} ${
                      rect_width * 0.6
                    }, ${rect_height} ${rect_width * 0.5}, ${
                      rect_height + rect_width * 0.1
                    } `
                  );
          ...
          function barEvent(bar) {
            let tooltipMove = (bar) => {
              ...
              let trans_x = newDataObj.barOption.orientation
                ? bar_x +
                  subjectScale.bandwidth() / 2 +
                  (seriesIndex * barWidth) / 2 +
                  (seriesIndex - !seriesIndex) * barOption.interval +
                  tooltip_width * 0.1
              ...
              let checkOverEdge = () => {
                ...
                if (trans_x + tooltip_width * 1.1 > width) {
                  ...
                  polygonPoints = `${tooltip_width}, ${
                    tooltip_height * 0.4
                  } ${tooltip_width}, ${tooltip_height * 0.6} ${
                    tooltip_width + tooltip_width * 0.1
                  }, ${tooltip_height / 2} `;
                } else
                  polygonPoints = `0, ${tooltip_height * 0.4} 0, ${
                    tooltip_height * 0.6
                  } ${-tooltip_width * 0.1}, ${tooltip_height / 2} `;
            ...
            let barHighLight = (bar, dir) => {
              ...
              switch (dir) {
                ...
                case 0:
                  ...
                  if (!beenClicked)
                    seriesGroup.selectAll(".bar").attr("fill-opacity", 1);
                  break;
                ...
                case 1:
                  seriesGroup
                    .selectAll(".bar")
                    .attr("fill-opacity", function () {
                      let isTarget = this == bar;
                      let beenClicked = this.classList.contains("clicked");
                      // // console.debug(this.classList.contains("clicked"));
                      // console.debug(isTarget, beenClicked)
                      // console.debug(bar)
                      if (!(isTarget || beenClicked)) return fadeOut;
                      else d3.select(bar).attr("stroke-opacity", 1);
                    });
                  break;
              }
            };
            let checkAllBarClicked = (barValue) => {
              let seriesIndex = parseInt(
                barValue / (subjects.length * categories.length)
              );
              let subjectIndex = barValue % subjects.length;
              // console.debug(seriesIndex, subjectIndex);
              let sameBarValueArr = categories.map(
                (d, i) =>
                  i * subjects.length +
                  subjectIndex +
                  seriesIndex * subjects.length * categories.length
            ...
            bar
              ...
              .on("click", function (e) {
                ...
                let clicked = bar.classed("clicked");
                // console.debug(clicked);
                ...
                } else tooltipGroup.select("#tooltip" + barValue).remove();
            ...
            tickCollection
              .on("click", function (e) {
                ...
                if (!clicked)
                  ...
                  newDataObj.newData.metric.forEach((series, seriesIndex) =>
                    makeTotalTooltip(subjectIndex, seriesIndex)
                  );
                else
                  dataKeys.forEach((series, seriesIndex) => {
        ...
        let chartOptionEvent = () => {
          //=====leftAxis option
          let leftAxisMetric = chartContainerD3.selectAll(
            'input[name ="leftAxisMetric"]'
          );
          let leftAxisMetricText = chartContainerD3.select(
            "#leftAxisOptionButton"
          );
          let leftAxisLog = chartContainerD3.select("#leftAxis_log");
          // console.debug(leftAxisLog);

          leftAxisMetric.on("change", (e) => {
          ...
          //=====rightAxis option
          let rightAxisMetric = chartContainerD3.selectAll(
            'input[name ="rightAxisMetric"]'
          );
          let rightAxisMetricText = chartContainerD3.select(
            "#rightAxisOptionButton"
          );
          let rightAxisLog = chartContainerD3.select("#rightAxis_log");
          // console.debug(rightAxisLog);

          rightAxisMetric.on("change", (e) => {
            let value = e.target.value;
            let checked = e.target.checked;
            //＝＝＝單選,其他勾拿掉
            rightAxisMetric
          ...
          chartContainerD3
            .selectAll('input[name ="changeChart"]')
            .on("click", (e) => {
              // console.debug('change')
              //===for reset tooltip after chart change
              let barBeenClicked = barNodes.filter((bar) =>
                bar.classList.contains("clicked") ? true : false
              );
              let tickBeenClicked = subjectTickCollection
                .nodes()
                .filter((bar) =>
                  bar.classList.contains("clicked") ? true : false
                );
              ...
              newDataObj = getNewData({
                orientation: +(changeChart === "vertical"),
              });
              let transDuration = updateChart(true);
              //=== reset tooltip
              // tooltipGroup.selectAll("g[id^='totalTooltip']").remove();
              tickBeenClicked.forEach((tick) =>
                d3.select(tick).dispatch("click")
              ); //remove totalTooltip
              //   console.debug(transDuration);
              d3.timeout(() => {
                barBeenClicked.forEach((bar) => {
                  tooltipGroup.select("#tooltip" + bar.value).remove();
                  d3.select(bar).dispatch("mouseover").dispatch("click");
                });
                tickBeenClicked.forEach((tick) =>
                  d3.select(tick).dispatch("click")
                ); //make totalTooltip
          ...
          // console.debug(chartContainerD3.select('input[name ="changeChart"]'))
          chartContainerD3
            .selectAll('input[name ="display"]')
            .on("click", (e) => {
              let checked = e.target.checked,
                value = e.target.value;

              if (checked) displayArr.push(value);
              else displayArr = displayArr.filter((val) => val !== value);
              newDataObj = getNewData({ displayArr: displayArr });
              updateChart();
              //   console.debug(displayArr, newDataObj.displayArr);
            });
          ...
          chartContainerD3
            ...
            .on("change", (e) =>
              svg
                .selectAll(".legend")
                .attr("display", e.target.checked ? "inline" : "none")
            );
        };
