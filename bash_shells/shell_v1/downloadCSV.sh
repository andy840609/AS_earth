...
if [ -z "$1" ]  
then 
echo "sh multi_scp_download.sh conf_file_path";  
exit;  
fi  
...
for((i=0;i<${#ssh_hosts[@]};i++))  
	...
	if [ "$ssh_port" = "" ]  
	then 
	ssh_port=${ssh_ports[0]}; #use default value  
	fi  
	#remote ssh user 
	ssh_user=${ssh_users[$i]};  
	if [ "$ssh_user" = "" ]  
	then 
	ssh_user=${ssh_users[0]}; #use default value  
	fi  
	#remote ssh password 
	ssh_password=${ssh_passwords[$i]};  
	if [ "$ssh_password" = "" ]  
	then 
	ssh_password=${ssh_passwords[0]}; #use default value  
	fi  
	#scp file or dir 
	########要分開資料夾用這兩句
	if [ "$scp_use_dir" = "true" ]
	then
	scp_local_host=$scp_local_dir_host/$ssh_host/  
	mkdir -p $scp_local_host
	/usr/bin/expect ./src/scp_download.sh "$ssh_host" "$ssh_port" "$ssh_user" "$ssh_password" "$scp_target_dir$scp_target" "$scp_local_host"    
	#########################
	else
	/usr/bin/expect ./src/scp_download.sh "$ssh_host" "$ssh_port" "$ssh_user" "$ssh_password" "$scp_target_dir$scp_target" "$scp_local_dir_host" 
	echo "" 
	fi
	else 
