            ...
            var appendPie = (dataGroup) => {
                ...
                pie
                    ...
                    .call(text => text.filter(d => !isSmallPie(d))
                        ...
                        .text(" " + (dataKey[dataGroup] == "times" ? "次" : dataKey[dataGroup]))
                    );
                ...
                function relax(label) {
                    ...
                    for (let i = 0; i < allPies.length; i++) {
                        ...
                        if (allPies[i] == thisPie_node && allPies[i] != allPies.length - 1) {
                            var thisIndex = i;
                            continue;
                        }

                        let otherLabel_node = allPies[i].querySelector('.label');

                        //=========if there's other label in front then check collide.
                        if (otherLabel_node) {



                            let thisLabel = d3.select(thisLabel_node);
                            let otherLabel = d3.select(otherLabel_node);

                            // var thisLabelRadius = thisLabel.data()[0].labelRadius;
                            // var otherLabelRadius = otherLabel.data()[0].labelRadius;
                            // var RadiusDiff = thisLabelRadius - otherLabelRadius;

                            let thisLabel_text_position = thisLabel.select('polyline').node().points[2];
                            let otherLabel_text_position = otherLabel.select('polyline').node().points[2];

                            let textX_Diff = thisLabel_text_position.x - otherLabel_text_position.x;
                            let textY_Diff = thisLabel_text_position.y - otherLabel_text_position.y;

                            // console.debug(thisLabel_text_position, otherLabel_text_position);
                            // console.debug(textY_Diff, radius);

                            //=========thisLabel and otherLabel are too far to collide
                            // if (Math.abs(textX_Diff) > radius || Math.abs(textY_Diff) > radius)
                            //     continue;
                            // if (1)
                            //     continue;
                            // console.debug("TextY");
                            // console.debug(thisLabel_textY, otherLabel_textY);
                            // console.debug("Radius");
                            // console.debug(thisLabelRadius, otherLabelRadius);
                            // console.debug("RD=" + RadiusDiff, "TD=" + textY_Diff);


                            // console.debug(thisIndex, i);
                            // if (i - thisIndex > 1 || isNaN(thisIndex))
                            //     continue;

                            // console.debug(i -thisIndex);
                            // console.debug(thisIndex, i);
                            if (Math.abs(textY_Diff) < spacing) {
                                console.log("RELAXing");
                                console.debug(textY_Diff);

                                // console.debug(thisIndex, i);
                                // console.debug(thisLabel_text_position, otherLabel_text_position);
                                //=====觸發事件的label是否為要延伸的label
                                var isThisLabelRelax = thisIndex < i ? false : true;

                                if (isThisLabelRelax) {
                                    var relaxLabel = thisLabel;
                                    // var circleMove = true;
                                    var labelRadius = otherLabel.data()[0].labelRadius;
                                }
                                else {
                                    var relaxLabel = otherLabel;
                                    // var circleMove = false;
                                    var labelRadius = thisLabel.data()[0].labelRadius;
                                }

                                var circleMove = false;
                                // labelRadius = thisLabelRadius;
                                labelRadius += spacing;
                                labelMove(relaxLabel, labelRadius, 200, circleMove);

                            }
                            // else if (RadiusDiff >= 2 * spacing - 1) {
                            //     console.log("shrinking");

                            //     if (isThisLabelRelax) {
                            //         var labelRadius = thisLabelRadius;
                            //     }
                            //     else {
                            //         var labelRadius = otherLabelRadius;
                            //     }
                            //     labelRadius -= spacing;
                            //     labelMove(relaxLabel, labelRadius, 200, circleMove);
                            // }
