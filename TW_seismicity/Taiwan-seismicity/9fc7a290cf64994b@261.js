...
export default function define(runtime, observer) {
  ...
  main.define("initial date", function () {
    return (
      0
    )
  });
  ...
  main.variable(observer("chart")).define("chart", ["d3", "data", "topojson", "tw", "mutable date", "legend", "projection", "svg", "invalidation"], function (d3, data, topojson, tw, $0, legend, projection, svg, invalidation) {
    ...
    const depth_domain = [data[0].depth, data[0].depth];
    data.forEach((v, i) => {
  ...
  main.variable(observer("legendGroup")).define("lengendGroup", ["d3", "legend", "chart", "DOM"], function (d3, legend, chart, DOM) {
    ...
    var depthLegend = legendGroup.append("g")
      .attr("transform", `translate(${legend_translate})`)
      .append(() => legend({
        color: depth_range,
        title: "Depth (km)",
    ...
    console.debug(depthLegend.node());
    //===========rlegend=====================
  ...
  main.variable().define("data", ["d3", "projection"], async function (d3, projection) {
    // const projection = d3.geoMercator().center([121, 24]).scale(8000);

    const data = await d3.csv("./data/ALVEYYE.csv", d => {
      // const p = projection([parseInt(d.lon), parseInt(d.lat)]);
    ...
    //Lng:118~124 Lat:21~26
    // data = data.filter(function (d) {

    //   return d != "red"
    // });

    // window.alert(data[0].depth);
    // for (const d of data)
    //   console.log(moment(d.date).format());
    // window.alert(data.length);
    return data;
  ...
  main.variable().define("tw", ["d3"], async function (d3) {
    ...
    // const tw = await d3.json("https://raw.githubusercontent.com/g0v/twgeojson/master/json/twCounty2010.geo.json", function (geoShape) {
