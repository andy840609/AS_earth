function requestRate() {
  ...
  function chart() {
    ...
    const timeDataKeys = [
      "processing_time",
      "internal_transmission_time",
      "external_transmission_time",
      "global_reaction_time",
    ];
    const rateDataKeys = [
      "processing_speed",
      "internal_transmission_rate",
      "external_transmission_rate",
    ...
    const convert_download_unit = (
      ...
      if (unitAfter) {
        //unitBefore 單位轉換到 unitAfter
        let unitAfter_obj = getUnit(unitAfter);
        let ratio1 = getRatio(
          unitBefore_obj.unit1,
          unitAfter_obj.unit1,
          unit1,
          8
        );
        let ratio2 = getRatio(
          unitBefore_obj.unit2,
          unitAfter_obj.unit2,
          unit2,
          1024
        );
        ...
        //unitBefore 單位轉換到 value>=1或單位已是最小(b)為止 ,並給newUnit
    ...
    const floatTimes = (...args) => {
      ...
      let result =
        intArr.reduce((a, b) => a * b) /
        Math.pow(
          10,
          powerArr.reduce((a, b) => a + b)
        );
      return result;
    };
    const makeShape = (
      ...
      let shape = { 0: "circle", 1: "rect", 2: "polygon", 3: "polygon" }[
        shapeIndex
      ];
      ...
      let shapeSelection = data //==沒有data的是legend上或是display menu的範例圖形
        ? d3Selection
            .selectAll("." + display_class)
            .data([data])
            .join(shape)
            .attr("class", display_class)
        : d3Selection.append(shape).attr("stroke", color ? color : "grey");
      ...
      switch (shapeIndex) {
        ...
        case 2:
          ...
          let triangle_points = polygonPoints(
            centre.x,
            centre.y,
            triangle_r,
            triangle_corner,
        ...
        case 3:
          // console.debug('star');
          let star_r1_points = polygonPoints(
            centre.x,
            centre.y,
            star_r1,
            star_corner,
          ...
          let star_r2_points = polygonPoints(
            centre.x,
            centre.y,
            star_r2,
            star_corner,
          ...
          shapeSelection
            ...
            .attr(
              ...
              d3
                ...
                .map(
                  (i) =>
                    star_r1_points[i].join(",") +
                    " " +
                    star_r2_points[i].join(",")
                )
            )
            .attr("fill-opacity", fillOpacity)
            .attr("stroke-opacity", strokeOpacity);
      ...
      let initData = () => {
        {
          ...
          typeName = typeName.filter(
            (tn) => !typesOfException.includes(tn[typeNameKeys[0]])
          );
          timeIntervalData = (function () {
            //==原資料爲ms所以除1000轉s
            let timeIntervals = data.map((d) => {
              let getTimeInterval = (date1, date2) => {
                let time_interval = date2 - date1;
                time_interval =
                  time_interval > 0 ? time_interval / 1000 : undefined;
          ...
          rateData = (function () {
            let getRate = (fileSize, ti) => {
              let rate =
                ti > 0 ? parseFloat((fileSize / ti).toFixed(2)) : undefined;
              return rate;
            };
            let rateData = timeIntervalData.map((d, i) => {
              let fileSize = data[i][dataKeys[1]];
              let ti1 = d[timeDataKeys[0]],
                ti2 = d[timeDataKeys[1]],
                ti3 = d[timeDataKeys[2]],
                ti4 = d[timeDataKeys[3]];
    ...
    let printChart = () => {
        ...
        chartDropDown.forEach((option) => {
          ...
          item.addEventListener("click", (e, a) => {
            ...
            let svg = chartContainerJQ
              ...
              .children("svg")[0];
            svgArr.push(svg);
            let chartVal = encodeURI(
              chartContainerJQ.find("input[name=chartType]:checked").val()
            );
      ...
      let chartType = document.querySelector(
    ...
    let getFileData = () => {
              ...
              rows.forEach((row, i) => {
                if (row != "") {
                  ...
                  if (i == 0)
                    ...
                    col.forEach(
                      (c, index) =>
                        (obj[fileDataKey[index]] = getParseData(c, index))
                    );
                    tmpData.push(obj);
                  }
                }
      ...
      dataPath.forEach((path) => {
        ...
        let dataArr =
          fileType == "json"
            ? readJsonFile(path, "request_log")
            : readTextFile(path, "request_log");
    ...
    function RQRchart() {
      ...
      function getNewData(chartOption) {
        ...
        let xAxisOption = chartOption.hasOwnProperty("xAxisOption")
            ? chartOption.xAxisOption
            : newDataObj.xAxisOption,
          yAxisOption = chartOption.hasOwnProperty("yAxisOption")
            ? chartOption.yAxisOption
            : newDataObj.yAxisOption,
          xAxisDomain = chartOption.hasOwnProperty("xAxisDomain")
            ? chartOption.xAxisDomain
            : newDataObj.xAxisDomain,
          displayArr = chartOption.hasOwnProperty("displayArr")
            ? chartOption.displayArr
            : newDataObj.displayArr,
          showArr = chartOption.hasOwnProperty("showArr")
            ? chartOption.showArr
            : newDataObj.showArr;
        ...
        if (
          chartOption.hasOwnProperty("xAxisOption") ||
          chartOption.hasOwnProperty("xAxisDomain")
        )
        ...
        if (newDataObj) userChartOption[0] = tmpObj; //==記得設定
        return tmpObj;
      }
      function updateChart(trans = false) {
            ...
            legendGroup
              ...
              .attr(
                ...
                `translate(${
                  ...
                  margin.right -
                  typeName.length *
                    (typeLegend_rect_width + typeLegend_rect_interval)
                }, ${margin.top * 0.7})`
              )
              .selectAll("g")
              .data(typeName)
              ...
              .attr(
                ...
                (d, i) =>
                  `translate(${
                    i * (typeLegend_rect_width + typeLegend_rect_interval)
                  }, 0)`
              )
              .call((g) => {
                g.append("rect")
                ...
                g.append("text")
                  ...
                  .attr(
                    "transform",
                    `translate(${typeLegend_rect_width / 2}, ${
                      typeLegend_rect_height + 2
                    })`
                  )
                  .attr("fill", "currentcolor")
                  .attr("color", "black")
                  ...
                  .call((text_collection) =>
                    text_collection.each(function (d) {
                      // console.debug(this);
            ...
            legendGroup
              ...
              .call((g) =>
                g
                  ...
                  .attr(
                    "height",
                    shapeLegend_eachShape_height * rateDataKeys.length
                  )
                  .attr("fill", "#D3D3D3")
                  .attr("opacity", 0.5)
                  .attr("stroke-width", "1")
              ...
              .attr(
                "transform",
                `translate(${
                  width - margin.right - shapeLegend_eachShape_width
                }, ${margin.top * 1.1})`
              )
              .selectAll("g")
              .data(rateDataKeys)
              ...
              .attr(
                "transform",
                (d, i) => `translate(0, ${i * shapeLegend_eachShape_height})`
              )
              .call((g_collection) =>
                g_collection.each(function (d, i) {
                  ...
                  makeShape(g, i, {
                    x: shapeLegend_rect_interval,
                    y: shapeLegend_rect_interval,
                  });
                  g.append("text")
        ...
        function render() {
                ...
                [].concat(
                  ...newData.map((d) =>
                    d3.extent(
                      [
                        display_DataKeys[0],
                        display_DataKeys[display_DataKeys.length - 1],
                      ],
                      (key) => d[key]
                    )
                  )
          ...
          let yDomain =
            display_rateDataKeys.length == 0
              ? d3.extent(
                  [].concat(
                    ...newRateData.map((rd) =>
                      d3.extent(rateDataKeys, (key) => rd[key])
                    )
                  )
                )
              : d3.extent(
                  [].concat(
                    ...newRateData.map((rd) =>
                      d3.extent(display_rateDataKeys, (key) => rd[key])
                    )
                  )
                );
          ...
          x = d3[
            {
              date: "scaleUtc",
              fileSize: xAxisOption.logScale ? "scaleLog" : "scaleLinear",
            }[xAxisOption.metric]
          ]()
            .domain(xDomain)
          ...
          y = d3[yAxisOption.logScale ? "scaleLog" : "scaleLinear"]()
            .domain(
              getNiceDomain(
                yDomain.map(
                  (d) =>
                    convert_download_unit(d, originUnit, yAxisOption.metric)
                      .value
                ),
                yAxisOption.logScale ? 0 : 0.01
              )
            )
              ...
              focusGroup
                ...
                .call(() =>
                  focusGroup.selectAll(".dots").each(function (d, i) {
                    ...
                    let rateArr = display_rateDataKeys.map(
                      (key) =>
                    ...
                    displayArr.forEach((shapeIndex, dataIndex) => {
                      ...
                      if (!isNaN(rateArr[dataIndex])) {
                        let centre = {
                          x: x(xdataArr[dataIndex]),
                          y: y(rateArr[dataIndex]),
                        };
                        makeShape(dots, shapeIndex, centre, rateArr[dataIndex]);
                      }
                      //否則刪除（不然會遺留之前group裡的圖形）
                      else dots.select("." + rateDataKeys[shapeIndex]).remove();
                    });
        ...
        if (!newDataObj) {
          newDataObj = userChartOption[0]
            ? userChartOption[0]
            : getNewData({
                xAxisOption: {
                  metric: chartContainerD3
                    .select("input[name=xAxisMetric]:checked")
                    .property("value"),
                  logScale: false,
                },
                yAxisOption: {
                  metric: chartContainerD3
                    .select("input[name=yAxisMetric]:checked")
                    .property("value"),
                  logScale: false,
                },
                xAxisDomain: null,
                displayArr: d3.range(rateDataKeys.length),
                showArr: chartContainerD3
                  .selectAll("input[name=show]:checked")
                  .nodes()
      ...
      function events(svg) {
          ...
          let mouseMove = () => {
            ...
            let updateTooltip = (dataArr) => {
              ...
              tooltip
                ...
                // .style('font-size', 10)
                .call((divCollection) =>
                  divCollection.each(function (obj, i) {
                    let div = d3.select(this);
                    ...
                    if (i != 0) {
                      div
                        .style(
                          "display",
                          display_rateDataKeys.includes(key) ? "block" : "none"
                        )
                        .selectAll("svg")
                        .data([0])
                        .join("svg")
                        ...
                        .call((displaySvg) => {
                          ...
                          makeShape(
                            displaySvg,
                            rateDataKeys.indexOf(key),
                            { x: svg_width * 0.5, y: svg_width * 0.5 },
                            1
                          );
                        });
                    }
                    ...
                    let text =
                      i == 0
                        ? key == dataKeys[2] //==datetime
                          ...
                          : convert_download_unit(value, originUnit, unit)
                              .value + `  ${unit} `
                        : parseFloat(value.toFixed(unit == "GB" ? 5 : 2)) +
                          `  ${unit}/s `;
            ...
            let hover = (target) => {
              ...
              focusGroup.selectAll("g").call((g) =>
                g.each(function (d, i) {
                  ...
                  g.attr(
                    "stroke",
                    hover ? (d) => getColor(d[dataKeys[0]]) : "grey"
                  )
                    .selectAll("*")
            ...
            focusGroup
              ...
              .on("mouseover", function (e) {
                ...
                let makeTooltip = () => {
                  ...
                  let dataArr = Array.from(dotGroup.children).map(
                    (child, i) => {
                      let obj = {};
                  ...
                  dataArr.sort(function (a, b) {
                    ...
                    if (Object.keys(b).length > 1) return 1;
                    else return Object.values(b) - Object.values(a);
                  });
        ...
        function chartOptionEvent() {
          //=====xaxis option
          let xAxisMetric = chartContainerD3.selectAll(
            'input[name ="xAxisMetric"]'
          );
          ...
          xAxisMetric.on("change", (e) => {
            ...
            xAxisOption.logScale = metricIsDate
              ? false
              : xAxisLog.property("checked");
          ...
          display.on("change", (e) => {
            ...
            d3.selectAll("." + rateDataKeys[display_index]).attr(
              "display",
              display
            );
          ...
          show.on("change", (e) => {
            ...
            d3.selectAll("." + showClassGroup[show_index]).attr(
              "display",
              display
            );
          ...
          if (userDataObj) {
            ...
            display
              .property("checked", false)
              .filter(function () {
                return userDisplayArr.includes(parseInt(this.value));
              })
              .property("checked", true);
            ...
            show
              .property("checked", false)
              .filter(function () {
                return userShowArr.includes(parseInt(this.value));
              })
              .property("checked", true);
          }
        }
          ...
          let raiseAndDrag = (d3_selection) => {
            ...
            let legend_dragBehavior = d3
              ...
              .on("drag end", function (e) {
                ...
                let yRange = [
                  targetFix + range_margin,
                  height - targetHeight + targetFix - range_margin,
                ];
                //不能拉出svg範圍
    ...
    function barChart() {
      ...
      ~(function init() {
        ...
        linearScale_group
          ...
          .on("mouseleave", function (e) {
            if (!textBoxFocus) linearScale_dropdownMenu.removeClass("show");
            menuMouseover = false;
          });
        ...
        let gapOptionList_html = gapMultiplicandArr
          .map((d, i) => `<option value="${i}">${d}</option>`)
          .join("");
        linearScale_dropdownMenu
          .find("#gapOptionList")
          .append(gapOptionList_html);
      })();
      ...
      function getNewData(chartOption) {
        let rateOption = chartOption.hasOwnProperty("rateOption")
            ? chartOption.rateOption
            : newDataObj.rateOption,
          metric = chartOption.hasOwnProperty("metric")
            ? chartOption.metric
            : newDataObj.metric,
          gap = chartOption.hasOwnProperty("gap")
            ? chartOption.gap
            : newDataObj.gap,
          logScale = chartOption.hasOwnProperty("logScale")
            ? chartOption.logScale
            : newDataObj.logScale;
          ...
          rateData.forEach((d) => {
            ...
            if (val)
              rateOptionData.push(
                metric == "KB"
                  ? val
                  : convert_download_unit(val, originUnit, metric).value
              );
          });
        ...
        let updateScaleMenu = (multiplier, minMultiplicand) => {
          ...
          linearScale_group
            .select("#gapOptionRange")
            .property("value", gapMultiplicandArr.indexOf(minMultiplicand));
          ...
          gapOptions
            ...
            .style("color", (d, i) =>
              gapMultiplicandArr[i] < minMultiplicand ? "grey" : "black"
            );
        ...
        let getGap = () => {
          ...
          let minMultiplicand = gapMultiplicandArr.find(
            (multiplicand) =>
              maxDomain / (multiplicand * multiplier) <= maxGroupCount
          );
        ...
        let getGapGroupData = (gap) => {
          ...
          rateOptionData = rateOptionData
            ? rateOptionData
            : newDataObj.rateOptionData;

          //==用不同方法計算分組陣列
          if (logScale) {
            let powerDomain = rateOptionData.domain.map((d, i) =>
              Math[i == 0 ? "floor" : "ceil"](Math.log10(d))
            );
            let groupCount = powerDomain.reduce((pre, cur) =>
              Math.abs(pre - cur)
            );
        ...
        if (
          chartOption.hasOwnProperty("rateOption") ||
          chartOption.hasOwnProperty("metric")
        )
          gap = getGap();
        ...
        if (newDataObj) userChartOption[1] = tmpObj;
        return tmpObj;
      }
      function updateChart(trans = true) {
        ...
        function render() {
          ...
          y = d3[logScale ? "scaleLog" : "scaleLinear"]()
            .domain(
              logScale ? gapGroupData.logDomain : [0, gap * gapGroupData.length]
            )
            .range([height - margin.bottom, margin.top]);
            ...
            let makeYAxis = (g) =>
              g
                ...
                .call((g) => {
                  ...
                  if (logScale) {
                    axisFun.ticks(
                      Math.log10(y.domain()[1] / y.domain()[0]),
                      formatPower
                    );
                  } else {
                    let tickValues = [0].concat(
                      gapGroupData.map((d, i) => floatTimes(gap, i + 1))
                    );
                    ...
                    if (gap.toString().indexOf(".") >= 0) {
                      ...
                      tickValues = tickValues.map((val) =>
                        val.toFixed(gapDecimal)
                      );
                    }
                ...
                .call((g) =>
                  g.selectAll("g.yAxis g.tick line").call((lines) => {
                    let tickLines = logScale
                      ? lines.filter((d) => Math.log10(d) % 1 === 0)
                      : lines;
            ...
            let bar = focusGroup
              ...
              .attr(
                ...
                (d, i) =>
                  y(
                    logScale
                      ? gapGroupData.logDomain[0] * Math.pow(10, i + 1)
                      : (i + 1) * gap
                  ) +
                  height * 0.5
              )
        ...
        if (!newDataObj) {
          ...
          newDataObj = userChartOption[1]
            ? userChartOption[1]
            : getNewData({
                rateOption: 3,
                metric: chartContainerD3
                  .selectAll("input[name=metric]:checked")
                  .property("value"),
      ...
      function events(svg) {
          ...
          let mouseMove = () => {
            ...
            let updateTooltip = (groupIndex, data) => {
              tooltip
                ...
                .attr(
                  "class",
                  "d-flex flex-wrap justify-content-center align-items-end"
                )
                .call((divC) =>
            ...
            let hover = (target) => {
              ...
              focusGroup.selectAll(".bar").call((barC) =>
                barC.each(function (d, i) {
                  let bar = d3.select(this);
                  let hover = this == target;

                  bar.attr("opacity", hover ? 1 : 0.5);

                  //===加陰影和上移圖層
                  if (hover) bar.attr("fill", "red").attr("stroke-opacity", 1);
            ...
            focusGroup
              ...
              .on("mouseover", function (e) {
                ...
                let makeTooltip = () => {
                  ...
                  let groupIndex = Array.from(bar.parentNode.children).indexOf(
                    bar
                  );
        ...
        function chartOptionEvent() {
          ...
          let linearScale_group = chartContainerD3.select("#linearScale_group");
          linearScale_group.select(".dropdown-menu").call((menu) => {
            // console.debug(menu);
            let gapScaleTextBox = menu.select("#gapNumber");
            let gapOptionRange = menu.select("#gapOptionRange");

            gapOptionRange.on("input", (e) => {
              //==========================同步textBox =================================
              let optionsIdx = e.target.value;
              // console.debug(optionsIdx)
              let minMultiplicandArr_idx = gapMultiplicandArr.indexOf(
                newDataObj.rateOptionData.minMultiplicand
              );
              ...
              let gap = floatTimes(
                gapMultiplicandArr[optionsIdx],
                newDataObj.rateOptionData.multiplier
              );
              // console.debug(gap)

              gapScaleTextBox.property("value", gap).dispatch("blur");
            });

            gapScaleTextBox.on("blur", (e) => {
              //==========================target vaule check=================================
          ...
          if (userDataObj) {
            ...
            d3.selectAll(metric.nodes().map((node) => node.parentNode))
              ...
              .filter(function () {
                return this.children[0].value == userMetric;
              })
              .classed("active", true)
              .select("input")
              .dispatch("click");
            ...
            linearScale_group.select(".dropdown-menu").call((menu) => {
              ...
              //==set range
              let gapOption = gapOptions.filter(function () {
                return this.text == userGap;
              });

              if (!gapOption.empty())
                gapOptionRange.property("value", gapOption.property("value"));

              linearScale_group.select("sub").text(`B.W. = ${userGap}`);

              gapScaleTextBox.property("value", userGap).dispatch("blur");
            ...
            d3.selectAll(scale.nodes().map((node) => node.parentNode))
              .classed("active", false)
              .filter(function () {
                return this.children[0].value == userLogScale;
              })
              .classed("active", true)
              .call((label) =>
                userLogScale ? label.select("input").dispatch("click") : null
              );
          }
        }
        chartEvent();
      ...
      function getNewData(chartOption) {
        let xAxisOption = chartOption.hasOwnProperty("xAxisOption")
          ...
          yAxisOption = chartOption.hasOwnProperty("yAxisOption")
            ? { ...chartOption.yAxisOption }
            : newDataObj.yAxisOption,
          selectedDomain = chartOption.hasOwnProperty("selectedDomain")
            ? chartOption.selectedDomain
            : newDataObj.selectedDomain,
          showArr = chartOption.hasOwnProperty("showArr")
            ? chartOption.showArr
            : newDataObj.showArr;
        ...
        let getDataArr = (key) => {
          ...
          if (selectedDomain) {
            ...
            newDataArr = timeIntervalData.map(
              (d, i) =>
                new Object({
                  typeId: data[i][dataKeys[0]],
                  requestTime: data[i][dataKeys[2]],
        ...
        let newDataArr =
          !newDataObj ||
          chartOption.hasOwnProperty("xAxisOption") ||
          chartOption.hasOwnProperty("selectedDomain")
        ...
        if (newDataObj) userChartOption[2] = tmpObj;
        // console.debug(tmpObj);
        return tmpObj;
      }
      function updateChart(trans = false) {
        ...
        function render() {
          ...
          let xDomain = selectedDomain
            ? selectedDomain.xDomain.map((d) =>
                d == 0 ? (xAxisOption.logScale ? 1 : 0) : d
              ) //==避免選玩的範圍有0轉log出錯
            : xAxisOption.logScale
            ? d3.extent(
                timeIntervalData,
                (d) => d[timeDataKeys[xAxisOption.metric]]
              )
            : [
                0,
                d3.max(
          ...
          let yDomain = selectedDomain
            ? selectedDomain.yDomain.map((d) =>
                d == 0
                  ? yAxisOption.logScale
                    ? convert_download_unit(
                        d3.min(newDataArr, (d) => d.fileSize),
                        originUnit,
                        yAxisOption.metric
                      ).value
                    : 0
              ...
              focusGroup
                ...
                .call(() => {
                  ...
                  focusGroup.selectAll(".dots").each(function (d, i) {
                    ...
                    // 有time就更新圖形
                    if (!isNaN(time)) {
                      let fileSize = convert_download_unit(
                        d.fileSize,
                        originUnit,
                        yAxisOption.metric
                      ).value;
        ...
        if (!newDataObj) {
          newDataObj = userChartOption[2]
            ...
            : getNewData({
                ...
                yAxisOption: {
                  metric: chartContainerD3
                    .selectAll("input[name=yAxisMetric]:checked")
                    .property("value"),
                  logScale: false,
                },
                selectedDomain: null,
            ...
            let updateTooltip = (data) => {
              ...
              let fileSize = parseFloat(
                convert_download_unit(
                  ...
                  originUnit,
                  unit
                ).value.toFixed(unit == "GB" ? 5 : 2)
              );
              let time = data.time;
              let rate = parseFloat(
                (fileSize / time).toFixed(unit == "GB" ? 5 : 2)
              );

              let typeString = typeName.find(
                (type) => type[typeNameKeys[0]] == data.typeId
              )[typeNameKeys[1]];
              let requestDate = new Date(data.requestTime)
                .toISOString()
                .split(".")[0];
              ...
              tooltip
                ...
                .attr(
                  "class",
                  "d-flex flex-nowrap justify-content-start align-items-end"
                )
                .style("white-space", "nowrap")
                .call((divC) =>
                  divC.each(function (d, i) {
