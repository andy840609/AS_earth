  ...
  constructor(UIkey, preScene, gameObj = null) {
    ...
    const tooltip = {
      tooltipHandler: (create = true, data = {}) => {
        if (create) {
          ...
          let x =
            obj.x +
            obj.displayWidth * (0.5 - obj.originX) +
            (data.dx ? data.dx : 0);
          let y =
            obj.y +
            obj.displayHeight * (1 - obj.originY) +
            18 +
            (data.dy ? data.dy : 0);
          let hotKeyString = gameData.controllCursor[obj.name]
            ? gameData.controllCursor[obj.name]
            : false;
          let text =
            UItextJSON[data.text ? data.text : obj.name] +
            (hotKeyString ? `(${hotKeyString})` : "");
          let tweensDuration = 200;

          //===background img
          let img = this.add
            .image(x, y + 1, data.img)
            .setFlipY(data.filpY ? data.filpY : false)
            .setOrigin(
              data.originX ? data.originX : 0.5,
              data.originY ? data.originY : 0.5
            )
            .setDepth(Depth.tooltip);

          //===tooltip text
          let tooltip = this.add
            .text(0, 0, text, {
          ...
          tooltip.setPosition(
            x +
              tooltip.width *
                0.75 *
                (0.5 - (data.originX ? data.originX : 0.5)) *
                2,
            y
          );
          img.setScale(
            0,
            (tooltip.height * (data.scaleY ? data.scaleY : 1.2)) / img.height
          );
          ...
          this.tweens.add({
            ...
            scaleX: {
              from: (tooltip.width * 0.1) / img.width,
              to: (tooltip.width * 1.5) / img.width,
            },
          });
    ...
    switch (UIkey) {
      case "iconBar":
        ...
        create = () => {
          ...
          this.iconButtons = UIButtonArr.map((button, i) => {
            ...
            let iconButton = this.add
              .image(
                barX + barWidth * (1 - (i + 1) / (buttonCount + 1)),
                barY + barHeight * 0.5,
                key
              )
              .setDepth(Depth.UI)
            ...
            iconButton
              ...
              .on("pointerdown", () => {
                ...
                if (!this.game.scene.getScene(key))
                  this.scene.add(null, new UIScene(key, this), true);
                else if (this.scene.isSleeping(key)) this.scene.wake(key);
                else this.scene.sleep(key);
        ...
        update = () => {
          let updateBar = () => {
            ...
            let orb2 = orbs[1];
            let isAllActive =
              orb1.laserObj.active &&
              !orb1.beholdingFlag &&
              orb2.laserObj.active &&
              !orb2.beholdingFlag;
            let isDiffTime =
              orb1.orbStats.time.toFixed(2) != orb1.originTime &&
            ...
            if (isAllActive && isDiffTime) {
              ...
              this.tweens.add({
                ...
                x: {
                  ...
                  to: (target, key, value, i, length) =>
                    iconBar.x +
                    eachButtonW * (length * (1 - (i + 1) / (length + 1)) - 1),
                },
              });
              ...
              //==SFX
              gameScene.sound.play("sfx_levelClear");
            }
          };
          let hotkeyPress = () => {
            ...
            UIButtonArr.forEach((button) => {
              if (
                Phaser.Input.Keyboard.JustDown(
                  cursors[gameData.controllCursor[button]]
                )
              ) {
      ...
      case "pauseUI":
        ...
        const doctorUI =
          gameScene.name != "boss"
            ? gameScene.game.scene.getScene("doctorUI")
            : null;
        const detectorUI =
          gameScene.name != "boss"
            ? gameScene.game.scene.getScene("detectorUI")
            : null;
        ...
        create = () => {
          ...
          const buttonGap =
            (height * menuH_scale - 2 * menuMarginY) / (buttons.length + 1);
          ...
          let buttonGroup = buttons.map((button, i) => {
            ...
            let buttonText = this.add
              .text(menu.x, y, UItextJSON[button], {
                font: "40px Arial",
                fill: "#ffffff",
              })
              .setOrigin(0.5)
            ...
            menuButton
              ...
              .setAlpha(
                button == "tutorial" && gameScene.name != "defend" ? 0.5 : 1
              )
              .setInteractive({ cursor: "pointer" })
              .on("pointerover", function () {
              ...
              .on("pointerdown", () => {
                switch (button) {
                  ...
                  case "setting":
                    ...
                    //==pauseUI被暫停,所以RexScrollablePanel要在另個scene
                    let settingUI = this.scene.add(
                      null,
                      new Phaser.Scene("settingUI"),
                      true
                    );
          ...
          this.tweens.add({
            ...
            scaleX: {
              from: 0,
              to: (target) => (buttonGroup[0].text.height * 2) / target.height,
            },
          });
      ...
      case "detectorUI":
        ...
        let initDetector = (screen = true) => {
          ...
          if (screen)
            this.detectorScreen = this.add
              .image(x + 0.5, y - 22, "detectorScreen")
              .setOrigin(0.5)
              .setScale(detectorScale)
        ...
        if (gameObj && gameObj.name == "tutorialDetector") {
          ...
          create = () => {
            ...
            let initOverview = () => {
              ...
              let wave = this.add
                .image(x + dx + 1, y + dy - 25, "tutorial_overview_waveForm")
                .setDepth(Depth.detector + 1);

              wave.setScale(
                screen.displayWidth / wave.displayWidth,
                (screen.displayHeight / wave.displayHeight) * 0.9
              );
            };
            let initBrushes = () => {
              let scaleFun = preScene.waveForm.overviewSvgObj.x.range([
                handleXMin,
                handleXMax,
              ]);

              let brushRect = this.add
                .rectangle(rectX2, rectY2, rectW2, rectH2, 0xea7500)
                .setDepth(Depth.detector + 2)
                .setOrigin(0)
                .setAlpha(0.3);

              let brushHandle1 = this.add
                .rectangle(handleXMin, rectY2, handleW, rectH2, 0x000000)
                .setDepth(Depth.detector + 3)
                .setOrigin(0)
                .setAlpha(0.5)
                .setName(0);

              let brushHandle2 = this.add
                .rectangle(handleXMax, rectY2, handleW, rectH2, 0x000000)
                .setDepth(Depth.detector + 3)
                .setOrigin(0)
                .setAlpha(0.5)
                .setName(1);
              ...
              let dragBehavior = (brush) => {
                brush
                  .setInteractive({ draggable: true, cursor: "col-resize" })
                  .on("drag", function (pointer, dragX, dragY) {
                    let newX;
                    if (dragX < handleXMin) newX = handleXMin;
                    else if (dragX > handleXMax) newX = handleXMax;
                    else newX = dragX;
                    this.x = newX;
                    updateBrushRect();

                    let domain = [
                      scaleFun.invert(brushHandle1.x),
                      scaleFun.invert(brushHandle2.x),
                    ].sort((a, b) => a - b);
              ...
              let updateBrushRect = () => {
                let newRectW = Phaser.Math.Distance.BetweenPoints(
                  brushHandle1,
                  brushHandle2
                );
                brushRect.x =
                  Math.min(brushHandle1.x, brushHandle2.x) + handleW * 0.5;
              ...
              let updateWave = (domain = null, scaleY = 1) => {
                let action = () => {
                  ...
                  gameScene.waveForm
                    .getWaveImg(
                      preScene.waveForm.tutorialData,
                      domain,
                      false,
                      scaleY
                    )
                ...
                if (
                  preScene.stepObj.nowStep == 4 &&
                  !preScene.buttonGroups.buttonWobbleTween
                )
                  ...
                  case "functionKey":
                    ...
                    btnFun = () => {
                      if (button.name == "functionKey") {
                        ...
                        if (nowIdx >= button.block.maxIdx)
                          button.block.nowIdx = 0;
                      } else {
                        let brushHandle =
                          button.name == "shiftLeft"
                            ? brushHandle1
                            : brushHandle2;
              ...
              const scaleBtn1X = x + dx + 43,
                scaleBtn1Y = handle1BtnY - 20;
              scaleButtonName.forEach((d, i) => {
                let scaleButton = this.add
                  .image(scaleBtn1X, scaleBtn1Y + i * 45, d)
                  .setScale(buttonScale)
                  .setDepth(Depth.detector + 5)
                  .setName(d);
              ...
              //===振幅倍率
              let scaleText = this.add
                .text(scaleBtn1X, scaleBtn1Y + 20, "1", {
              ...
              let resetButton = this.add
                .image(x + dx + 85, handle1BtnY, "resetButton")
                .setScale(buttonScale)
                .setDepth(Depth.detector + 5)
                .setName("reset");
          ...
          update = () => {
            let updateButton = () => {
              ...
              detectorButtons.forEach((button) => {
                let condition =
                  button.name == "shiftLeft" || button.name == "shiftRight"
                    ? cursors[gameData.controllCursor[button.name]].isDown
                    : Phaser.Input.Keyboard.JustDown(
                        cursors[gameData.controllCursor[button.name]]
                      );
          ...
          create = () => {
            ...
            const scaleFun = gameScene.waveForm.overviewSvgObj.x.range([
              handleXMin,
              handleXMax,
            ]);
            ...
            let initOverview = () => {
              ...
              let wave = () => {
                let screen = this.detectorScreen;
                let wave = this.add
                  .image(x + 1, y - 25, "overview_waveForm")
                  .setDepth(Depth.detector + 1);
            ...
            let initBrushes = () => {
              ...
              let brushRect = this.add
                .rectangle(rectX, rectY, rectW, rectH, 0xea7500)
                .setDepth(Depth.detector + 2)
                .setOrigin(0)
                .setAlpha(0.3);

              let brushHandle1 = this.add
                .rectangle(handleXMin, rectY, handleW, rectH, 0x000000)
                .setDepth(Depth.detector + 3)
                .setOrigin(0)
                .setAlpha(0.5);

              let brushHandle2 = this.add
                .rectangle(handleXMax, rectY, handleW, rectH, 0x000000)
                .setDepth(Depth.detector + 3)
                .setOrigin(0)
                .setAlpha(0.5);
              ...
              let updateWave = (
                ...
                let action = () => {
                  ...
                  gameScene.waveForm
                    .getWaveImg(stationData, domain, false, scaleY)
                    .then((success) => {
                      // console.debug(success)
                      //==避免波形沒更新到
                      new Promise((resolve, reject) => {
                      ...
                      this.orbs.forEach((orb) => {
                        ...
                        let outOfWindow = !this.cameras.main.worldView.contains(
                          orb.x,
                          orb.y
                        );
              ...
              //==保留上次選取範圍
              let xAxisDomain = gameScene.waveForm.domain
                ? gameScene.waveForm.domain
                : null;
              if (xAxisDomain) {
              ...
              const handle1BtnX = x - 91,
                handle1BtnY = y + 86;
              handleButtonName.forEach((d, i) => {
                let handleButton = this.add
                  .image(handle1BtnX + i * 45, handle1BtnY, d)
                  .setScale(buttonScale)
                  .setDepth(Depth.detector + 5)
                  .setName(d);
              ...
              //===重置按鈕
              let resetButton = this.add
                .image(x + 90, handle1BtnY, "resetButton")
                .setScale(buttonScale)
                .setDepth(Depth.detector + 5)
                .setName("reset");
              detectorButtons.push(resetButton);
              ...
              this.events.on("playerMove", (moveX) => {
                ...
                //==沒超出螢幕範圍才更新
                if (
                  checkInRange(brushHandle1.x) &&
                  checkInRange(brushHandle2.x)
                ) {
            ...
            let initMapIcon = () => {
              ...
              this.orbIcons = this.orbs.map((orb) => {
                ...
                let orbIcon = this.add
                  .sprite(orbX, y + 25, "orb")
                  .setOrigin(0.5)
                  .setScale(0.1)
                  .setDepth(Depth.detector + 2);
          ...
          update = () => {
            ...
            let updateIcon = () => {
              this.orbs.forEach((orb, i) => {
                ...
                if (orb.beholdingFlag) this.orbIcons[i].updatePos();
              });
            };
            updateButton();
            updateIcon();
          ...
          create = () => {
            let initOverview = () => {
              ...
              let initMinimap = () => {
                let createMinimap = () => {
                  this.minimap = gameScene.cameras
                    .add(rectX, rectY, rectW, rectH)
                    .setScene(this)
              ...
              let initScreenRect = () => {
                ...
                this.screenRect = this.add
                  .rectangle(rectX, rectY, sRectW, sRectH, 0x0066cc)
                ...
                let dragBehavior = (rect) => {
                  ...
                  rect
                    ...
                    .on("drag", function (pointer) {
                      ...
                      // console.debug(dragX);
                      updateMainCamera(
                        (this.x + 0.5 * sRectW - rectX) / mapZoom,
                        (this.y + 0.5 * sRectH - rectY) / mapZoom
                      );
                    });
                };
      ...
      case "timerUI":
        ...
        create = () => {
          ...
          let initTimer = () => {
            ...
            if (
              gameScene.name != "boss" &&
              gameScene.firstTimeEvent.isFirstTime
            )
              //==說話時暫停
          ...
          let initBox = () => {
            ...
            timeString.forEach((d, i) => {
              ...
              let label = this.add
                .text(
                  barX + x + blockW * 0.5,
                  barY + blockMargin + blockW,
                  UItextJSON[timeString[i]],
              ...
              //==time Text
              let timeText = (this.gameTimer.timeText[d] = this.add
                .text(
                  barX + x + blockW * 0.5,
                  barY + blockMargin + blockW * 0.5,
                  "120",
          ...
          let initHourglass = () => {
            ...
            let scale = 0.4;
            let hourglass = this.add
              .sprite(barX + 33, barY + 30, 0, 0, "hourglass")
              .setScale(scale)
              .setOrigin(0.5)
              .setDepth(Depth.UI)
              .setName("hourglass")
        ...
        update = () => {
          ...
          let updateHourglassAnime = () => {
            ...
            if (timeVal < speedUP) {
              let msPerFrame =
                min_msPerFrame +
                (this.hourglass.max_msPerFrame - min_msPerFrame) *
                  (timeVal / speedUP);
      ...
      case "depthCounterUI":
        ...
        create = () => {
          let x = 20,
            y = 300;
          let initRuler = () => {
            this.depthRuler = this.add
              .image(x, y, "depthRuler")
              .setScale(0.3, 0.2)
              .setOrigin(0, 0.5)
              .setAlpha(0.9)
              .setDepth(Depth.UI);
          ...
          let initCounter = () => {
            gameScene.depthCounter.text = this.add
              .text(x + this.depthRuler.displayWidth * 0.7, y, "", {
                fontSize: "32px",
                fill: "#000",
              })
              .setOrigin(0.5)
      ...
      case "doctorUI":
        ...
        create = () => {
          this.doctor = this.add
            .existing(new Doctor(this, gameScene.gameData.localeJSON.Doctor))
            .setDepth(Depth.UI);
        ...
        update = () => {
          ...
          //==玩家靠近變透明
          let playerApproach =
            Phaser.Math.Distance.BetweenPoints(gameScene.player, this.doctor) <
            320;
          ...
          let updatePlayer = () => {
            ...
            if (
              ...
              player.statsChangeHandler(
                { HP: playerStats.healthRegen / 100 },
                gameScene
              ); //回血
            //==狀態對話框
            player.dialog.setPosition(
              player.x,
              player.y - player.displayHeight * 0.3
            );
          };
          updatePlayer();
        };
      ...
      case "statsBar":
        ...
        if (gameObj.name == "player") {
          ...
          create = () => {
            ...
            let initBox = () => {
              hpBox = this.add
                .image(BoxX, hpBoxY, "UIbar_bar")
                .setScale(1.5)
                .setOrigin(0)
                .setDepth(Depth.box);
              mpBox = this.add
                .image(BoxX, mpBoxY, "UIbar_bar")
                .setOrigin(0)
                .setDepth(Depth.box);
              headBox = this.add
                .image(BoxX - 85, hpBoxY + 25, "UIbar_head")
                .setScale(1.5)
                .setOrigin(0, 0.5)
                .setDepth(Depth.headBox);

              this.add
                .image(BoxX, hpBoxY, "UIbar_HPlabel")
                .setScale(1.5)
                .setOrigin(0)
                .setDepth(Depth.label);
              this.add
                .image(BoxX, mpBoxY, "UIbar_MPlabel")
                .setScale(1.5)
                .setOrigin(0, 0.5)
                .setDepth(Depth.label);
              ...
              this.HPText = this.add
                .text(
                  BoxX + hpBox.displayWidth * 0.9,
                  hpBoxY + hpBox.displayHeight * 0.8,
                  "",
              ...
              this.MPText = this.add
                .text(
                  BoxX + mpBox.displayWidth * 0.9,
                  mpBoxY + mpBox.displayHeight * 0.8,
                  "",
            ...
            let initBar = () => {
              let getGradientColor = (gradientColor, percent) => {
                function hexToRgb(hexString) {
                  let result =
                    /^0x?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(
                      hexString
                    );
                  return result
                    ? {
                ...
                function rgbToHex(rgbObj) {
                  ...
                  return (
                    "0x" +
                    componentToHex(rgbObj.r) +
                    componentToHex(rgbObj.g) +
                    componentToHex(rgbObj.b)
                  );
                }
              ...
              let makeBar = (stats) => {
                ...
                Object.assign(bar, {
                  ...
                  updateBar: () => {
                    ...
                    //==update text
                    this[`${stats}Text`].setText(
                      `${parseInt(currentVal)} / ${totalVal}`
                    );
                  },
                });
                bar.updateBar();
            ...
            let initHead = () => {
              ...
              this.add
                .image(headX + headW * 0.5, headY, "playerAvatar")
                .setScale(0.5)
                .setOrigin(0.5)
                .setDepth(Depth.headBox)
                .setMask(mask);
          ...
          create = () => {
            ...
            let initBar = () => {
              const barW = hpBox.displayWidth * 0.88,
                barH = 15;
              const barX = -barW * 0.61,
                barY = -barH * 0.2;

              let bar = this.add.graphics().setDepth(Depth.bar);
          ...
          create = () => {
            let makeBar = () => {
              ...
              let bar = this.add.graphics().setDepth(Depth.UI);
        ...
        update = () => {
          ...
          let updateOrb = () => {
            ...
            gameScene.orbGroup.children.iterate((child) => {
              if (
                child.beholdingFlag ||
                child.laserUpdateFlag ||
                !child.body.touching.down
              ) {
            ...
            this.fadeInTween = this.tweens
              .add({
                ...
                alphaGL: (
                  ...
                  totalTargets,
                  tween
                ) => tween.custom.alphaGL,
                repeat: 0,
              })
              .stop(0);
            this.fadeOutTween = this.tweens
              .add({
                targets: camera.backgroundColor,
                // ease: 'Quadratic.InOut',
                duration: (
      ...
      case "RexUI": //==問答、對話框、可拉動內容框
        ...
        create = () => {
          let addRexUI = () => {
            ...
            this.newDialog = (content, config = null, resolve = null) => {
              ...
              let textBox = new RexTextBox(
                this,
                {
                  ...
                  pageendEvent: config.pageendEvent
                    ? config.pageendEvent
                    : false,
                },
                resolve
            ...
            //==開場字幕
            this.newTextPlayer = (config = null, resolve = null) => {
              return new RexTextPlayer(
                this,
                {
                  x: config.x ? config.x : DLconfig.dialogX,
          ...
          let guideArrow = () => {
            if (
              gameScene.name == "boss" ||
              !(
                gameScene.firstTimeEvent && gameScene.firstTimeEvent.isFirstTime
              )
            )
              return;
            let animsCreate = () => {
      ...
      case "creator": //==創角色畫面
        ...
        create = () => {
          ...
          let background = () => {
            ...
            let resources =
              BackGroundResources.GameStart[creatorObj.background];
            ...
            resources.dynamic.forEach((res, i) => {
              ...
              this.tweens.add(
                Object.assign(
                  ...
                  animType == 1
                    ? {
                        tilePositionX: { start: 0, to: thing.width },
                        ease: "Linear",
                      }
                    : animType == 2
                    ? {
            ...
            let charaSprites = characters.map((chara, i) => {
              ...
              let character = this.add
                ...
                .on("pointerdown", function () {
                  ...
                  scene.form.on("destroy", (form) => {
                    if (form.formConfirm) {
                      charaSprites.forEach((chara) =>
                        chara.removeInteractive()
                      );
              ...
              let getStatsBlock = (chara) => {
                ...
                let block = this.add
                  .image(character.x, character.y - 50, "statsBlock")
                  .setScale(0.4)
                  .setOrigin(0.5, 1)
                  .setDepth(Depth.UI + 2);
                ...
                stats.forEach((sta, i) => {
                  let text = this.add
                    .text(textX, textY + textH * i + 30, UItextJSON[sta], {
                  ...
                  starAmount =
                    starAmount < 1 ? 1 : starAmount > 3 ? 3 : starAmount;
                  Array.from(new Array(starAmount), (d, i) => i).forEach(
                    (i) => {
                      let star = this.add
                        .image(block.x + 5 + i * 30, text.y, "statsStar")
                        .setScale(0.5)
                        .setOrigin(0.5)
                        .setDepth(Depth.UI + 3);
                      group.add(star);
              ...
              let statsBlock = getStatsBlock(chara).setVisible(false);
          ...
          let title = () => {
            ...
            let gameTitle = this.add
              .image(width * 0.5, 0, "gameTitle")
              .setOrigin(0.5, 0)
              .setVisible(false)
              .setDepth(Depth.UI + 1);
            ...
            let subTitle = this.add
              .text(width * 0.5, 0, UItextJSON["chooseCharacter"], {
                fontSize: "48px",
                fill: "#000000",
                shadow: {
                  offsetX: 5,
            ...
            let showTitle = () => {
              ...
              this.tweens.add({
                ...
                ease: "Linear.Out",
                onStart: () =>
                  subTitle.setY(height * 0.1 + gameTitle.displayHeight),
              });
            };
      ...
      case "tutorial": //==教學關
        ...
        const pauseUI =
          gameScene.name != "GameStart"
            ? gameScene.game.scene.getScene("pauseUI")
            : null;
        ...
        preload = () => {
          let tutorialWindow = () => {
            let dir =
              assetsDir + "gameObj/environment/background/" + tutorialBG + "/";
          ...
          let controller = () => {
            ...
            this.load.spritesheet("sprinkle", UIDir + "sprinkle.png", {
              frameWidth: 300,
              frameHeight: 300,
            });
            this.load.image(
              "wf_plot",
              assetsDir + "ui/game/Transitions/wf_plot.png"
            );
            if (gameScene.name == "GameStart")
              this.load.image(
                "tooltipButton",
                assetsDir + "ui/game/tooltipButton.png"
              );
          };
          let player = () => {
            ...
            let sprite = () => {
              ...
              this.load.spritesheet(
                "player_jumpDust",
                effectDir + "jump_dust.png",
                { frameWidth: 38, frameHeight: 60 }
              );
              ...
              this.load.spritesheet(
                "player_jumpAttackEffect",
                dir + "jumpAttackEffect.png",
                {
              ...
              this.load.spritesheet(
                "player_runAttackEffect",
                dir + "runAttackEffect.png",
                {
                  frameWidth: effectFrameObj.run[0],
                  frameHeight: effectFrameObj.run[1],
                }
              );
              if (gameData.playerStats.class) {
                //遠程子彈
          ...
          let sidekick = () => {
            ...
            this.load.spritesheet(
              "sidekick_idle",
              dir + sidekick + "_Monster_Idle_4.png",
              frameObj
            );
            this.load.spritesheet(
              "sidekick_run",
              dir + sidekick + "_Monster_Run_6.png",
              frameObj
            );
            this.load.spritesheet(
              "sidekick_jump",
              dir + sidekick + "_Monster_Jump_8.png",
              frameObj
            );
            this.load.spritesheet(
              "sidekick_attack",
              dir + sidekick + "_Monster_Attack2_6.png",
              frameObj
            );
            //==dust
            this.load.spritesheet(
              "sidekick_jumpDust",
              dir + "Double_Jump_Dust_5.png",
              frameObj
            );
            this.load.spritesheet(
              "sidekick_runDust",
              dir + "Walk_Run_Push_Dust_6.png",
              frameObj
            );
          };
          let dummy = () => {
          ...
          let orb = () => {
            ...
            this.load.spritesheet("orb", dir + "orb.png", {
              frameWidth: 256,
              frameHeight: 256,
            });
            this.load.spritesheet("laser", dir + "laser.png", {
              frameWidth: 512,
              frameHeight: 682.6,
            });
          ...
          let wave = () => {
            ...
            this.waveForm = {
              ...
              overviewSvgObj: new Promise((r) => {
                ...
                //==getOverviewSVG
                gameScene.waveForm
                  .getWaveImg(tutorialData, null, true)
                  .then((success) => {
                    this.load.svg("tutorial_overview_waveForm", success.svg, {
                      scale: 1,
                    });
        ...
        create = () => {
          ...
          let tutorialWindow = () => {
            let frames = () => {
              let img = this.add
                .image(tutorialX, tutorialY, "frames")
                .setDepth(Depth.UI);
              img
                .setOrigin(0.508, 0.05)
                .setScale(
                  (tutorialW * 1.12) / img.width,
                  (tutorialH * 1.35) / img.height
                );
            };
            let button = () => {
              const buttons = [
                gameScene.name == "GameStart" ? "skip" : "close",
                "previous",
                "next",
                "info1",
                "info2",
              ...
              buttons.forEach((name, i) => {
                    ...
                    switch (name) {
                      case "skip":
                        ...
                        let confirmIdx = await new Promise((resolve) => {
                          let confirmScene = this.scene.add(
                            null,
                            new Phaser.Scene("confirmScene"),
                            true
                          );
                        ...
                        //==確認跳過
                        if (
                          questionData.options[confirmIdx] == UItextJSON["ok"]
                        ) {
                          gameScene.game.destroy(true, false);
                          gameScene.resolve(false);
                        } else return;
                      ...
                      case "info2":
                        let tooltipHandler =
                          this.detectorUI.tooltip.tooltipHandler;
                        this.buttonGroups.infoObjs.forEach((obj) =>
                          obj.destroy()
                        );
                        ...
                        if (!button.click) {
                          ...
                          if (name == "info1") {
                            ...
                            this.detectorUI.brushHandles.forEach((b) =>
                              b.disableInteractive()
                            );
                            this.detectorUI.detectorButtons.forEach((b) =>
                              b.disableInteractive()
                            );
                          ...
                          this.buttonGroups[anotherButton]
                            .getChildren()
                            .find((c) => c.type === "Image").click = false;
                        }
                ...
                this.buttonGroups[name] = this.add
                  .group()
                  .add(button)
                  .add(buttonText);
            ...
            let text = () => {
              ...
              this.stepText = this.add
                .text(tutorialX, tutorialY + tutorialH + 50, "", {
                  fontSize: "24px",
                  fill: "#000000",
                  padding: {
                    top: 3,
              ...
              this.clearText = this.add
                .text(width * 0.5, height * 0.5, UItextJSON["clear"], {
                  fontSize: "200px",
                  fill: "#EB8529",
                  stroke: "#EA8D33",
                  strokeThickness: 10,
              ...
              Object.assign(this.clearText, {
                ...
                showHandler: () => {
                  const showDuration = 1000;
                  if (this.clearText.showingTween)
                    this.clearText.showingTween.remove();
            ...
            let stepHandler = () => {
              ...
              this.stepHandler = (flash = true) => {
                let step = this.stepObj.nowStep,
                  ...
                  stepText = UItextJSON["stepText"]
                    .replace("\t", this.stepObj.nowStep)
                    .replace("\t", this.stepObj.maxStep);
                ...
                if (flash) {
                  ...
                  this.player.enableBody(
                    true,
                    ObjOrigin.x + tutorialW * 0.2,
                    ObjOrigin.y + tutorialH * 0.5,
                    true,
                    true
                  );
                  if (this.player.pickUpObj) {
                    //==有減光球就放下
                    this.player.pickUpObj.statusHadler(
                      this.player,
                      false,
                      false
                    );
                  ...
                  this.sidekick.enableBody(
                    true,
                    ObjOrigin.x + tutorialW * 0.1,
                    ObjOrigin.y + tutorialH * 0.6,
                    true,
                    true
                  );
                  if (this.sidekick.talkingCallback) {
                    if (this.sidekick.talkingTween)
                      this.sidekick.talkingTween.remove();
                  ...
                  this.buttonGroups.buttonWobble(
                  ...
                  [
                    this.buttonGroups["info1"],
                    this.buttonGroups["info2"],
                  ].forEach((info, i) => {
                ...
                switch (step) {
                  case 1:
                    ...
                    leftKey = UItextJSON[leftKey]
                      ? UItextJSON[leftKey]
                      : leftKey;
                    rightKey = UItextJSON[rightKey]
                      ? UItextJSON[rightKey]
                      : rightKey;
              ...
              animsCreate();
              this.sprinkle = this.add
                .sprite(width * 0.5, height * 0.5, "sprinkle")
                .setVisible(false)
                .setDepth(Depth.UI);

              this.sprinkle
                .setScale(
                  (tutorialW * 1.2) / this.sprinkle.width,
                  (tutorialH * 1.2) / this.sprinkle.height
                )
                .on("stepClear", function () {
                  this.scene.buttonGroups.buttonWobble(
                    this.scene.buttonGroups["next"]
                  );
          ...
          let initPlayer = () => {
            ...
            Object.assign(this.player, {
              ...
              movingHadler: function (scene) {
                ...
                let isBusy =
                  ((currentAnims === "player_runAttack" ||
                    (currentAnims === "player_jumpAttack" &&
                      !this.body.touching.down)) &&
                    this.anims.isPlaying) ||
                  (currentAnims === "player_doubleJump" &&
                    !this.body.touching.down);
                ...
                if (cursors[gameData.controllCursor["left"]].isDown) {
                  ...
                  if (!this.body.onWall())
                    scene.background.forEach(
                      (bg, i) => (bg.tilePositionX -= 0.3 * (i + 1))
                    );

                  if (isBusy) return;
                  this.anims.play(
                    !this.body.touching.down ? "player_jump" : "player_run",
                    true
                  );
                ...
                if (
                  Phaser.Input.Keyboard.JustDown(
                    cursors[gameData.controllCursor["up"]]
                  )
                ) {
                  //==SFX
                  scene.sound.play("sfx_playerJump");
                  ...
                  //==二段跳
                    ...
                    this.anims.getName() === "player_jump" &&
                    this.doublejumpFlag
                  ) {
                    this.setVelocityY(-this.stats.jumpingPower);
              ...
              pickingHadler: function (scene) {
                ...
                if (
                  Phaser.Input.Keyboard.JustDown(
                    cursors[gameData.controllCursor["down"]]
                  )
                ) {
                  // console.debug('pick');
                  if (this.pickUpObj) {
                    ...
                    this.pickUpObj = null;
                    if (
                      scene.stepObj.nowStep == 3 &&
                      !scene.buttonGroups.buttonWobbleTween
                    )
                      scene.sprinkle.emit("stepClear");
                  } else {
                    //==pick up
                        ...
                        if (colsestOrb)
                          colsestOrb =
                            ...
                            Phaser.Math.Distance.BetweenPoints(this, colsestOrb)
                              ? child
                              : colsestOrb;
                        else colsestOrb = child;
                    });
                    if (colsestOrb) {
                      //==SFX
                      scene.sound.play("sfx_playerPickUp");
              ...
              attackHandler: function (scene) {
                if (scene.stepObj.nowStep == 1 || scene.stepObj.nowStep == 4)
                  return;
                let cursors = gameScene.cursors;
                ...
                if (cursors[gameData.controllCursor["attack"]].isDown) {
                  ...
                  let attacking =
                    (currentAnims === "player_attack" ||
                      currentAnims === "player_runAttack" ||
                      currentAnims === "player_jumpAttack" ||
                      currentAnims === "player_specialAttack") &&
                    this.anims.isPlaying;
                  ...
                  let attackAnims = isJumping
                    ? "player_jumpAttack"
                    : isRuning
                    ? "player_runAttack"
                    : "player_attack";
                  let attackEffectAnims = isJumping
                    ? "player_jumpAttackEffect"
                    : isRuning
                    ? "player_runAttackEffect"
                    : "player_attackEffect";
                  ...
                  if (currentAnims === "player_attack" && this.anims.isPlaying)
                    return;
                  this.anims.play(attackAnims);
                  //==SFX
                  scene.sound.play(
                    attackAnims === "player_runAttack"
                      ? "sfx_playerSlide"
                      : "sfx_playerAttack"
                  );
                  ...
                  if (bullet) {
                    if (this.stats.class)
                      bullet
                        .play(
                          isRuning ? "player_bullet2" : "player_bullet1",
                          true
                        )
            ...
            //===init attack
            this.player.bullets = this.physics.add
              .group({
                classType: Bullet,
            ...
            this.player.attackEffect = this.add
              .sprite(0, 0)
              .setScale(2)
              .setOrigin(0.5, 0.4)
          ...
          let initSidekick = () => {
            this.sidekick = this.add
              .existing(new Sidekick(this, gameData.sidekick.type))
              .setPosition(
                ObjOrigin.x + tutorialW * 0.1,
                ObjOrigin.y + tutorialH * 0.6
              );
          ...
          let initDummy = () => {
            ...
            Object.assign(this.dummy, {
              ...
              statsBarUI: this.scene.add(
                null,
                new UIScene("statsBar", this, this.dummy),
                true
              ),
              deadTweens: null,
              ...
              statsChangeHandler: function (statsObj, scene) {
                ...
                this.statsChangeCallback = scene.time.delayedCall(
                  1500,
                  () => {
                    scene.tweens.add({
                      targets: this.HPbar,
          ...
          let environment = () => {
              ...
              resources.static.forEach((res, i) => {
                ...
                switch (i) {
                  ...
                  case resources.static.length - 1:
                    ...
                    img = this.platforms.create(
                      tutorialX,
                      tutorialY + tutorialH - groundH,
                      "staticTutorialBG_" + i
                    );
                    img
            ...
            let initOrb = () => {
              ...
              this.orbGroup.children.iterate((child, i) => {
                ...
                //=laser
                child.laserObj = this.physics.add
                  .sprite(child.x, child.y + 20, "laser")
                  .setOrigin(0.5, 1)
                ...
                Object.assign(child, {
                  ...
                  statusHadler: function (
                    ...
                    if (beholding) {
                      ...
                      this.setDepth(Depth.pickUpObj).anims.play(
                        "orb_holded",
                        true
                      );
                    } else {
                      ...
                      this.setDepth(Depth.orbs).anims.play(
                        activate ? "orb_activate" : "orb_inactive",
                        true
                      );
                    ...
                    if (pickUper) {
                      ...
                      if (beholding) {
                        ...
                        //==放下後角色屬性恢復
                        let originStas =
                          GameObjectStats.player[gameData.playerRole];
                        newCharacterStats = {
            ...
            let initWave = async () => {
              let wave = this.add
                .image(tutorialX, tutorialY + tutorialH * 0.6)
                .setDepth(Depth.wave)
                .setAlpha(0.7)
                .setVisible(false);
              ...
              // ===detector
              this.detectorUI = this.scene.add(
                null,
                new UIScene("detectorUI", this, {
        ...
        update = () => {
          ...
          let updateOrb = () => {
            ...
            this.orbGroup.children.iterate((child, i) => {
              if (
                ...
                if (child.beholdingFlag) {
                  ...
                  let xAxisRange = [
                    (width - waveObj.displayWidth) * 0.5 +
                      margin.left * waveObj.scaleX,
                    (width + waveObj.displayWidth) * 0.5 -
                      margin.right * waveObj.scaleX,
                  ];
            ...
            if (
              this.stepObj.nowStep == 5 &&
              !this.buttonGroups.buttonWobbleTween
            )
              if (
                this.orbGroup
                  .getChildren()
                  .every((child) => child.body.touching.down)
              )
      ...
      case "backpackUI": //===道具
        ...
        create = () => {
          ...
          this.backpack = new RexPlugins.UI.Sizer(this, {
            ...
            .addBackground(
              this.add.existing(
                new RexPlugins.UI.RoundRectangle(
                  this,
                  0,
                  0,
                  0,
          ...
          const leftPannel = new RexPlugins.UI.Sizer(this, {
              orientation: 1,
              space: { item: 10 },
            }),
            rightPannel = new RexPlugins.UI.Sizer(this, {
              orientation: 1,
              space: { item: 10 },
            });
          ...
          const createMenu = (scene, gameObject, itemType = 0, itemIdx = 0) => {
            ...
            let menuScene = scene.scene.add(
              null,
              new Phaser.Scene("menuScene"),
              true
            );
            scene.scene.pause();
            ...
            switch (itemType) {
              case 0:
                ...
                options = [
                  ...
                  {
                    name: "hotkey",
                    children: [...Array(hotKeyAmount).keys()].map(
                      (i) => new Object({ name: "setHotkey" })
                    ),
                  },
                  { name: "detail" },
                ];
                ...
                break;
              case 1:
                options = [{ name: "equip" }, { name: "detail" }];
                break;
              case 2:
                options = [{ name: "unequip" }, { name: "detail" }];
                break;
            }
                  ...
                  switch (button.name) {
                    ...
                    case "unequip":
                      ...
                      player.equip
                        ...
                        .setScale(
                          (player.displayWidth * 1.5) / player.equip.width
                        );
                      ...
                      let statChangeHandler = (item, isEquip = true) => {
                        ...
                        let tmp = {};
                        //算裝備或脫下要加還減
                        Object.keys(buffAbility).forEach(
                          (key) =>
                        ...
                        if (item === "syringe") {
                          ...
                          if (pickUpObj && pickUpObj.changeStats) {
                            Object.keys(pickUpObj.changeStats).forEach(
                              (key) =>
                                (tmp[key] +=
                                  pickUpObj.changeStats[key] *
                                  (isEquip ? 1 : -1))
                            );
                          }
                          ...
                          if (isEquip && player.slowDownTween) {
                            ...
                            player.slowDownTween.callbacks.onComplete.func(
                              player.slowDownTween
                            );
                      ...
                      if (gameScene.enemy)
                        if (
                          equipItem === "scientistCard" ||
                          backpackData.onEquip.includes("scientistCard")
                        ) {
                          gameScene.enemy.children.iterate((child) => {
                            // console.debug(child.behavior);
                            child.behavior =
                              equipItem === "scientistCard" && isEquip
                                ? "worship"
                                : "";
                          });
                        }
          ...
          const createIcon = (scene, config) => {
            ...
            let item = config.item
              ? scene.add.image(0, 0, "item_" + config.item)
              : false;
            ...
            let badgeLabel = new RexPlugins.UI.BadgeLabel(scene, {
              ...
              rightBottom: config.isEquip
                ? false
                : scene.add
                    .text(0, 0, config.amount, {
                      color: "#fff",
                      align: "right",
          ...
          let itemBlock = (pannel) => {
            ...
            this.updateItems = () => {
              ...
              if (itemIndex === -1) {
                ...
                //==數量大於0改變數字
                if (amount > 0) {
                  let amountText = table
                    .getCellContainer(itemIndex)
                    .getElement("icon")
            ...
            const table = new RexPlugins.UI.GridTable(this, {
              ...
              .on(
                "cell.click",
                function (cellContainer, cellIndex, pointer) {
                  if (preClickItem && preClickItem._active)
                    preClickItem.getElement("background").setStrokeStyle();
          ...
          let equipBlock = (pannel) => {
            ...
            const table = new RexPlugins.UI.GridTable(this, {
              ...
              .on(
                "cell.click",
                function (cellContainer, cellIndex, pointer, e) {
                  if (cellContainer.name === "") return;
          ...
          let charaBlock = (pannel) => {
            const table = new RexPlugins.UI.Sizer(this, {
              orientation: 0,
            }).addBackground(
              this.add.existing(
                new RexPlugins.UI.RoundRectangle(
            ...
            let updateCharaPic = (onEquipData) => {
              ...
              if (equip) {
                ...
                equipImage.setTexture("onEquip_" + equip).setVisible(true);
            ...
            let initCharaTable = () => {
              ...
              let getCharaPic = () => {
                ...
                //==照片背景(邊角要變圓不然超出框線）
                let backGround = new RexPlugins.UI.CircleMaskImage(
                  this,
                  0,
                  0,
                  "charaBG" + background_idx,
                  {
                ...
                backGround.setScale(
                  charaPic.width / backGround.width,
                  charaPic.height / backGround.height
                );
              ...
              let charaName = new RexPlugins.UI.Label(this, {
                text: this.add
                  .text(0, 0, gameData.playerCustom.name, {
                    fontSize: "24px",
                    fixedWidth: 100,
                    align: "center",
                    padding: padding,
          ...
          let statusBlock = (pannel) => {
            ...
            let getBuffEffect = (key) => {
              ...
              let buffString =
                buffVal === 0
                  ? ""
                  : `(${gameData.playerStats[key]}${
                      buffVal > 0 ? "+" : ""
                    }${buffVal})`;
              let color =
                buffVal === 0 ? "black" : buffVal < 0 ? "red" : "blue";
      ...
      case "hotKeyUI": //==道具快捷鍵
        ...
        const hotKeyAmount = 3,
          hotKeyButtons = [...Array(hotKeyAmount).keys()].map(
            (i) => "hotkey" + (i + 1)
          );
        const blockW = 50;
        ...
        create = () => {
          ...
          let createIcon = (hotkeyIdx) => {
            ...
            let backpackItem = item
              ? itemData.find((backpackItem) => backpackItem.name === item)
              : false;
            ...
            return new RexPlugins.UI.BadgeLabel(this, {
              ...
              rightBottom: item
                ? this.add
                    .text(0, 0, backpackItem ? backpackItem.amount : 0, {
                      color: "#fff",
                      align: "right",
                      backgroundColor: "#474747",
                      padding: { left: 1, right: 1, top: 2, bottom: 0 },
            ...
            //==道具數量變化同時改變快捷鍵顯示
              ...
              hotKeyData.forEach((hotkeyItem, hotkeyIdx) => {
                if (hotkeyItem === itemName) {
                  let backpackItem = itemData.find(
                    (backpackItem) => backpackItem.name === itemName
                  );
          ...
          this.useItemHandler = (itemIdx) => {
            ...
            switch (gameItemData.type) {
              case 0: //是消耗品
                Object.keys(gameItemData.buff).forEach((key) =>
                  gameScene.player.statsChangeHandler({
                    [key]: gameItemData.buff[key],
                  })
                );
                break;
              case 1: //是放置物品
            ...
            // console.debug('是放置物品');
            //==數量用完就刪除
            if ((itemData[itemIdx].amount -= 1) <= 0)
              itemData.splice(itemIdx, 1);
        ...
        update = () => {
          let updateButton = () => {
            ...
            hotKeyButtons.forEach((hotKey, i) => {
              if (
                Phaser.Input.Keyboard.JustDown(
                  cursors[gameData.controllCursor[hotKey]]
                )
              ) {
                // console.debug(hotKey);
                let itemIdx = itemData.findIndex(
                  (backpackItem) => backpackItem.name === hotKeyData[i]
                );
...
class GameStartScene extends Phaser.Scene {
  ...
  preload() {
    ...
    let callback = (resolve) =>
      this.scene.add(null, new LoadingScene(this, resolve), true);
  ...
  create() {
...

    {
      let initBackground = () => {
        ...
        let gameTitle = () => {
          let gameTitle = this.add
            .image(width * 0.3, height * 0.5, "gameTitle")
            .setRotation(Math.random());

          gameTitle.spinningHandler = function () {
            this.setRotation(this.rotation - 0.01).setScale(
              0.6 + Math.abs(Math.sin(this.rotation))
            );
          };
                  ...
                  panel.on("destroy", () => {
                    ...
                    if (button == "setting")
                      buttonGroup.forEach((group, i) =>
                        group.text.setText(
                          this.gameData.localeJSON.UI[buttons[i]]
                        )
                      );
                    this.scene.resume();
                  });
    ...
    let initStartAnime = () => {
      ...
      let getChildByName = (name) => {
        return this.sceneGroup.getChildren().find((obj) => obj.name === name);
      };
      ...
      let initScene = () => {
            ...
            async () => {
              let showEmote = (character, emoji) => {
                if (emoji)
                  this.tweens.add({
                    ...
                    ease: "Quadratic.InOut",
                    onStart: () => {
                      character.emote.setTexture(emoji).setVisible(true);
                    },
                  });
                else
                  this.tweens.add({
              ...
              for (let i = 0; i < Object.keys(lines).length; i++) {
                ...
                switch (i) {
                  ...
                  case 6:
                    //==SFX
                    this.sound.play("sfx_playerAttack");
                    this.RexUI.cameras.main.shake(animeDelay * 4, 0.03);
                    this.player
                      .play("player_death")
                      .anims.stopOnFrame(
                        this.player.anims.currentAnim.frames[1]
                      );
                  ...
                  case 7:
                    ...
                    await new Promise((resolve) =>
                      this.time.delayedCall(animeDelay * 4, () => resolve())
                    );
                    //     break;
                    // case 9:
                    ...
                    this.time.delayedCall(animeDelay * 4, () => {
                      //==SFX
                      this.sound.play("sfx_bossAttack");
                      this.tweens.add({
                        targets: effectScene.effect1,
                        alpha: { from: 1, to: 0 },
                        duration: animeDelay * 2,
                        ...
                        onComplete: () => {
                          ...
                          effect2
                            .play("boss_effect2")
                            .setScale(
                              (this.boss.x - width) / effect2.width,
                              1.5
                            );
                    ...
                    blackOut.fadeOut();
                    await new Promise((resolve) =>
                      this.time.delayedCall(animeDelay * 13, () => {
                ...
                let dialogCode = await new Promise((resolve) =>
                  this.RexUI.newDialog(
                    line,
                    { character, pageendEvent },
                    resolve
                  )
                );
        ...
        let scene1 = () => {
          ...
          let apple = () => {
            let appleTree = this.add
              .image(width * 0.6, height * 0.45, "appleTree")
              .setScale(0.6)
              .setName("appleTree")
              .setDepth(Depth.appleTree);

            let apple = this.add
              .image(appleTree.x - 90, height * 0.4, "apple")
              .setScale(0.6)
              .setName("apple")
        ...
        let scene2 = () => {
          ...
          let planet = () => {
            ...
            animsCreate();
            let earth = this.add
              .sprite(width * 0.5, height * 0.5, "planetEarth")
              .setScale(1.2)
              .setOrigin(0.5, 0.4)
              .setName("earth")
          ...
          let animStart = () => {
            playSubtitle(1, "jump").then(() => {
              ...
              this.doctor.emote.setPosition(this.doctor.x, this.doctor.y - 70);
      ...
      let initPlayer = () => {
        let animsCreate = () => {
          ...
          this.anims.create({
            key: "player_think",
            frames: this.anims.generateFrameNumbers(player + "_attack", {
              frames: [0],
            }),
            frameRate: frameRate.attack,
            repeat: 0,
      ...
      let initSidekick = () => {
        ...
        this.doctor = this.add.sprite(0, 0).setScale(2).setDepth(Depth.player);

        this.doctor.emote = this.add
          .image(0, 0)
          .setScale(1.5)
          .setVisible(false)
          .setDepth(Depth.player);
      ...
      let initBoss = () => {
        let animsCreate = () => {
          this.anims.create({
            key: "boss_flyAttack",
            frames: this.anims.generateFrameNumbers("boss_flyAttack"),
            frameRate: 3,
            repeat: 0,
          });
          ...
          this.anims.create({
            key: "boss_effect1",
            frames: this.anims.generateFrameNumbers("boss_effect1"),
            frameRate: 8,
            repeat: -1,
          });
        ...
        this.boss = this.add
          .sprite(width * 1.8, height * 0.4, "boss_flyAttack")
          .setScale(3)
          .setFlipX(true)
          .setDepth(Depth.boss);

        Object.assign(this.boss, {
          flyTweens: this.tweens.add({
            targets: this.boss,
            repeat: -1,
            ease: "Quadratic.InOut",
            duration: animeDelay,
          ...
          effectScene: this.scene.add(
            null,
            new Phaser.Scene("bossEffect"),
            true
          ),
        });
        ...
        Object.assign(effectScene, {
          effect1: effectScene.add
            .sprite(this.boss.x + 40 - width, this.boss.y - 40)
            .setScale(1.5)
            .setDepth(Depth.boss + 1),
          effect2: effectScene.add
            .sprite(this.boss.x - 100 - width, this.boss.y - 40)
            .setFlipX(true)
            .setOrigin(1, 0.5)
            .setDepth(Depth.boss + 1),
    ...
    let initAudio = () => {
      let music = this.sound.add("mainTheme", {
        volume: 1,
        loop: true,
        delay: 0,
      });
    ...
    let initEndAnime = () => {
      ...
      let initScene = () => {
        ...
        let playSubtitle = (index = 0, animes = false, padding = null) => {
          ...
          return textPlayer.playPromise(text).then(() => textPlayer.destroy());
        };
        ...
        let credits = async () => {
          ...
          for (let i = 0; i < lineLength; i++) {
            const anim =
              i !== lineLength - 1 ? anims[i % anims.length] : "theEnd";
            let padding = {
              top: height * 0.1,
              left: width * 0.4,
            };
        ...
        let playerRunning = (part = 1) => {
          ...
          switch (part) {
            ...
            case 3: //==跑進家
              ...
              x = {
                from: this.player.x,
                to: houseX + this.house.displayWidth * 0.54,
              };
              duration = animeDelay * 4;
              delay = duration;
              onComplete = () => {
              ...
              this.tweens.add({
                ...
                onComplete: () => {
                  this.sceneGroup2.parallaxTween.forEach((tween) =>
                    tween.stop()
                  );
                  const zoomDura = 500;
        ...
        let scenes = () => {
                ...
                default: //parallax
                  ...
                  let tween = this.tweens.addCounter({
                    ...
                    onStart: (tween) => {
                      tween.custom = {
                        tilePositionX:
                          this.sceneGroup1.parallaxShift * Math.pow(i + 1, 2),
                      };
                    },
                    onRepeat: (tween) => {
              ...
              this.tweens.add(
                Object.assign(
                  ...
                  animType == 1
                    ...
                    : animType == 2
                    ? { alpha: { start: 0, to: 1 }, ease: "Back", yoyo: true }
                    : animType == 3
                    ? {
                        scaleX: {
                          start: (t) => t.scaleX,
              ...
              switch (i) {
                default: //parallax
                  ...
                  let tween = this.tweens.addCounter({
                    ...
                    onRepeat: () =>
                      (img.tilePositionX +=
                        this.sceneGroup2.parallaxShift * Math.pow(i + 1, 2)),
                  });
          ...
          let object = () => {
            ...
            this.portal.setPosition(
              width + this.portal.displayWidth,
              this.player.y
            );
            //===
            this.house = this.add
      ...
      let initAudio = () => {
        let music = this.sound.add("endTheme", {
          volume: 1,
          // rate: 1,
          // detune: 0,
          // seek: 0,
...
class LoadingScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // console.debug(gameScene);
    const packNum = { GameStart: 0, defend: 1, dig: 2, boss: 3, GameOver: 4 }[
      gameScene.name
    ];
    let gameObjects = () => {
      let environment = () => {
        ...
        switch (packNum) {
          case 0:
            let scene = () => {
              ...
              let planet = () => {
                const planetW = 512;
                this.load.spritesheet("planetEarth", startDir + "earth.png", {
                  frameWidth: planetW,
                  frameHeight: planetW,
                });
                this.load.spritesheet("planetMars", startDir + "mars.png", {
                  frameWidth: planetW,
                  frameHeight: planetW,
                });
              };
          ...
          case 1:
            ...
            let wave = () => {
              ...
              let xAxisDomain = stationData.stationStats.orbStats
                ? stationData.stationStats.orbStats.xAxisDomain
                : null;

              //==getWaveSVG
              gameScene.waveForm
                .getWaveImg(stationData, xAxisDomain)
                .then((success) => {
              ...
              //==getOverviewSVG
              gameScene.waveForm
                .getWaveImg(stationData, null, true)
                .then((success) => {
          ...
          case 2:
            ...
            let mineObjs = () => {
              ...
              //==魔王門素材
              this.load.spritesheet("bossDoor", mineObjDir + "bossDoor.png", {
                frameWidth: 100,
                frameHeight: 100,
              });

              this.load.spritesheet("bossTorch", mineObjDir + "bossTorch.png", {
                frameWidth: 320,
                frameHeight: 320,
              });
            };
          ...
          case 3:
            let bossRoom = () => {
              ...
              this.load.spritesheet("bossFlame", castleDir + "flame.png", {
                frameWidth: 1000,
                frameHeight: 1000,
              });
            ...
            let boss = () => {
              ...
              this.load.spritesheet("boss_Attack", bossDir + "Attack.png", {
                frameWidth: 100,
                frameHeight: 100,
              });
              this.load.spritesheet("boss_Death", bossDir + "Death.png", {
                frameWidth: 100,
                frameHeight: 100,
              });
              this.load.spritesheet("boss_Fly", bossDir + "Fly.png", {
                frameWidth: 100,
                frameHeight: 100,
              });
              this.load.spritesheet("boss_Idle", bossDir + "Idle.png", {
                frameWidth: 100,
                frameHeight: 100,
              });
            };
          ...
          case 4:
            ...
            let clear = () => {
              let scene = () => {
                ...
                let gameObject = () => {
                  this.load.spritesheet("endPortal", endDir + "portal.png", {
                    frameWidth: 360,
                    frameHeight: 592,
                  });
      ...
      let player = () => {
        ...
        let sprite = () => {
          ...
          if (packNum == 3)
            this.load.spritesheet(
              "player_ultAttackEffect",
              effectDir + "ult_effect.png",
              {
      ...
      let sidekick = () => {
        ...
        let doctor = () => {
          this.load.image(
            "doctorOwl",
            assetsDir + "ui/map/sidekick/Doctor2.png"
          );
        };
        let sidekick = () => {
          const sidekick = gameData.sidekick.type;
      ...
      if (packNum === 0) {
        ...
        let character = () => {
          ...
          let sprite = () => {
            ...
            let doctor = () => {
              let dir = gameObjDir + "sidekick/doctor/";
              const frameObj = {
                frameWidth: 80,
                frameHeight: 60,
              };
            ...
            let boss = () => {
              ...
              this.load.spritesheet(
                "boss_flyAttack",
                bossDir + "flyAttack.png",
                { frameWidth: 100, frameHeight: 100 }
              );
              this.load.spritesheet("boss_effect1", bossDir + "effect1.png", {
                frameWidth: 320,
                frameHeight: 253,
              });
              this.load.spritesheet("boss_effect2", bossDir + "bossBeam.png", {
                frameWidth: 500,
                frameHeight: 230,
              });
            };
        ...
        let enemy = () => {
          ...
          gameScene.aliveEnemy.forEach((enemy) => {
            ...
            this.load.spritesheet(
              enemy + "_Attack",
              dir + "Attack.png",
              enemy == "dove" ? { frameWidth: 48, frameHeight: 48 } : frameObj
            );
            this.load.spritesheet(
              enemy + "_Death",
              dir + "Death.png",
            ...
            if (enemy === "dove") {
              this.load.spritesheet(enemy + "_goo", dir + "goo.png", {
                frameWidth: 58,
                frameHeight: 56,
              });
    ...
    let UI = () => {
      ...
      let detector = () => {
        ...
        this.load.image(
          "shiftLeft",
          dir + `shiftLeft${packNum === 2 ? "_dig" : ""}.png`
        );
        this.load.image(
          "shiftRight",
          dir + `shiftRight${packNum === 2 ? "_dig" : ""}.png`
        );
      };
      let backpack = () => {
        if (packNum === 0) return;
        ...
        let items = () => {
          ...
          //==裝備要額外讀一張圖
          itemsArr
            .filter((key) => GameItemData[key].type === 2)
            .forEach((key) =>
              this.load.image(
                "onEquip_" + key,
                itemsDir + "onEquip_" + key + ".png"
              )
            );
        };
      ...
      let timeRemain = () => {
        if (packNum === 0) return;
        this.load.spritesheet("hourglass", uiDir + "hourglass.png", {
          frameWidth: 200,
          frameHeight: 310,
        });
      ...
      let dialog = () => {
        ...
        let avatar = () => {
          ...
          switch (packNum) {
            case 1:
              if (gameScene.firstTimeEvent.isFirstTime) {
                this.load.image(
                  "enemyAvatar",
                  avatarDir + gameScene.aliveEnemy[0] + ".png"
                );
              }
              break;
            case 3:
          ...
          this.load.image(
            "playerAvatar",
            `${avatarDir + gameData.playerRole}/${
              gameData.playerCustom.avatarIndex
            }.png`
          );
          this.load.image(
            "sidekickAvatar",
            avatarDir + gameData.sidekick.type + ".png"
          );
        };
        textBox();
        quiz();
    ...
    let makeProgressBar = () => {
      ...
      let progressGraphics = () => {
        //==為了作dude動畫
        let loadDude = () => {
          this.load.spritesheet("dude", assetsDir + "ui/game/dude.png", {
            frameWidth: 32,
            frameHeight: 48,
          });
        };
      ...
      let loadEvents = () => {
        ...
        this.load.on("fileprogress", (file) => {
          this.assetText.setText(
            `${LoadtextJSON["LoadingAsset"]}: ${file.key}`
          );
        });
        ...
        this.load.on("filecomplete", (key) => {
          ...
          this.dude = this.add
            .sprite(this.progressBar.x, this.progressBar.y - 100, "dude")
            .play("dude_run");
        });
    ...
    let audio = () => {
      ...
      this.load.audio("mainTheme", audioDir + "BGM/mainTheme.wav");
      if (packNum === 3)
        this.load.audio("bossFightTheme", audioDir + "BGM/bossFight.wav");
      if (packNum === 4)
        this.load.audio("endTheme", audioDir + "BGM/endTheme.wav");
      ...
      SFXs.forEach((sfx) =>
        this.load.audio("sfx_" + sfx, audioDir + `SFX/${sfx}.wav`)
      );
    };
    makeProgressBar();
    gameObjects();
...
class DefendScene extends Phaser.Scene {
  constructor(stationData, GameData, other) {
    ...
    Object.assign(this, {
      ...
      getTimePoint: (x, getPosition = false) => {
        ...
        if (getPosition) {
          ...
          let isInRange =
            position >= xAxisRange[0] && position <= xAxisRange[1];
          statsObj = {
            time: x,
            isInRange: isInRange,
            position: position,
    ...
    this.aliveEnemy = Object.keys(enemyStats).filter(
      (enemy) => enemyStats[enemy].HP > 0
    );
  ...
  create() {
    ...
    let initEnvironment = () => {
      let station = () => {
        ...
        let img = this.add
          .image(width * 0.92, height * 0.53, "station")
          .setDepth(Depth.station);
        img.setScale(1, height / img.height);

        this.add
          .text(width * 0.88, height * 0.46, station, {
            fontSize: "32px",
            fill: "#000",
          })
          .setRotation(-0.1)
      ...
      let orbs = () => {
        ...
        this.orbGroup.children.iterate((child, i) => {
          ...
          child.setPosition(orbPosition, height * 0.8).setName("orb");
          child.body.setSize(100, 100, true);
          ...
          child.laserObj.setScale(
            0.3,
            (height * 0.9) / child.laserObj.displayHeight
          );

          child.laserObj.body.setMaxVelocityY(0).setSize(50);

          //=time
          // let timeString = activate ? (orbStats[i].timePoint.time).toFixed(2) : '';
          child.timeText = this.add
            .text(0, 0, "", { fontSize: "20px", fill: "#A8FF24" })
            .setOrigin(0.5)
            .setDepth(Depth.UI);
          ...
          child.hintBox.add(orbBox).add(hintOrb).setAlpha(0);
          ...
          Object.assign(child, {
            originTime: (orbStats
              ? this.getTimePoint(width * 0.85)
              : child.orbStats
            ).time.toFixed(2), //==用來判斷是否通關(位置要移動過)
            beholdingFlag: false,
            ...
            statusHadler: function (
              ...
              if (pickUper) {
                ...
                if (beholding) {
                  //==撿起後角色屬性改變
                  Object.keys(child.changeStats).forEach(
                    (key) => (changeStats[key] = -child.changeStats[key])
                  );
                } else {
                  //==放下後角色屬性恢復
                ...
                if (pickUper.name === "player") {
                  if (pickUper.slowDownTween) {
                    ...
                    let originalSpeed =
                      pickUper.stats["movementSpeed"] +
                      pickUper.slowDownTween.slowDownSpeed;
                    let newSlowDown =
                      (originalSpeed + changeStats["movementSpeed"]) *
                      pickUper.slowDownTween.slowDownRate;
                    // console.debug(originalSpeed, newSlowDown)
                    if (originalSpeed && newSlowDown) {
                      changeStats["movementSpeed"] +=
                  ...
                  Object.keys(pickUper.stats).forEach(
                    (key) => (pickUper.stats[key] += changeStats[key])
                  );
              }
            },
            outWindowHandler: function (outWindow) {
              ...
              this.hintBox.hintOrb.setPosition(
                hintBoxX + orbBox.displayWidth * 0.1 * (filp ? 1 : -1),
                hintBoxY
              );
            },
          });
      ...
      let wave = () => {
        let wave = this.add
          .image(width * 0.5, height * 0.5, "waveForm")
          .setDepth(Depth.wave)
          .setAlpha(0.7);
    ...
    let initSidekick = () => {
      let sidekick = () => {
        this.sidekick = this.add
          .existing(new Sidekick(this, this.gameData.sidekick.type))
          .setPosition(40, 500);
      ...
      this.enemy.enemyAttack = (player, foe) => {
        ...
        if (!player.invincibleFlag) {
          ...
          player.body.setVelocityX(
            knockBackSpeed * (foe.x < player.x ? 1 : -1)
          );
        ...
        if (child.bullets && child.bullets.name === "eggs") {
          ...
          child.bulletAttack = (player, bullet) => {
            ...
            if (this.gameData.backpack.onEquip.includes("pan")) {
              this.add.existing(
                new Item(this, "sunny", bullet.x, bullet.y, true)
              );
            }
            //==沒有則受到攻擊
            else {
              ...
              if (!player.invincibleFlag) {
                ...
                player.invincibleFlag = true;
                player.body.setVelocityX(
                  knockBackSpeed * (child.x < player.x ? 1 : -1)
                );
            ...
            (bullet, platform) => {
              ...
              this.time.delayedCall(
                ...
                () => {
                  ...
                  let goo = this.physics.add
                    .sprite(bullet.x, platform.y - 0.8 * platform.displayHeight)
                    .setScale(2)
                    .setOrigin(0.5, 1)
                    .setImmovable(true)
                  ...
                  // console.debug(goo);
                  let gooCollider = this.physics.add.overlap(
                    goo,
                    this.player,
                    () => {
                      // console.debug('aaa');
                      if (
                        this.player.slowDownTween ||
                        this.gameData.backpack.onEquip.includes("syringe")
                      )
                        return;
  ...
  update() {
              ...
              for (let i = 0; i < Object.keys(lines).length; i++) {
                ...
                switch (i) {
                  ...
                  case 1:
                    line = line
                      .replace("\t", playerName)
                      .replace("\t", this.gameData.localeJSON.UI[sidekickName]);
                    break;
                  //====================教學==========================================
                  case 2: //==0.人物操作說明
                    blackOut.scene.setVisible(false);
                  ...
                  case 4: //==2.說明探測器的zoom
                    ...
                    guideArrow
                      .setFlipX(false)
                      .setPosition(
                        detectorUI.detector.x - arrowWidth * 0.5,
                        detectorUI.detector.y
                      );
                    break;
                  case 5: //==3.快捷鍵
                    blackOut.scene.bringToTop();
                    ...
                    guideArrow
                      .setFlipX(false)
                      .setPosition(
                        hotKeyUI.hotKeyBar.x - arrowWidth * 1.1,
                        hotKeyUI.hotKeyBar.y - arrowHeight * 0.5
                      );
                  ...
                  case 6: //==4.HP/MP
                    ...
                    guideArrow
                      .setFlipX(true)
                      .setPosition(
                        playerUI.HPbar.x + arrowWidth * 2.5,
                        playerUI.HPbar.y + arrowHeight * 0.2
                      );
                  ...
                  case 7: //==5.time remain
                    ...
                    guideArrow
                      .setVisible(true)
                      .setPosition(
                        timerUI.hourglass.x + arrowWidth * 2,
                        timerUI.hourglass.y
                      );
                    break;
                  case 8:
                    ...
                    //==停頓在說
                    await new Promise((resolve) =>
                      this.time.delayedCall(speakDelay * 0.5, () => resolve())
                    );
                    pageendEvent = true;
                    break;
                  case 9:
    ...
    let updatePlayer = () => {
      ...
      let playerStats = this.player.stats;
      if (playerStats.MP < playerStats.maxMP)
        this.player.statsChangeHandler(
          { MP: playerStats.manaRegen / 100 },
          this
        ); //自然回魔
      ...
      //==狀態對話框
      this.player.dialog.setPosition(
        this.player.x,
    ...
    if (this.gameOver.flag) {
      ...
      //==時間到或死亡要對話框提示
      if (status != 0) {
        this.player.talkingHandler(
          this,
          this.gameData.localeJSON.UI["gameOver" + status],
          true
        );
      }
      ...
      //===get gameResult
      let orbStats = this.orbGroup
        .getChildren()
        .map((orb) => orb.orbStats)
      ...
      let gameResult = {
        ...
        stationInfo: {
          ...
          liberate: !(
            Object.keys(enemyStats).filter((enemy) => enemyStats[enemy].HP > 0)
              .length > 0
          ),
      ...
      this.gameOver.delayedCall = this.time.delayedCall(
        clear ? 0 : gameDestroyDelay,
        async () => {
          if (clear) {
            const canvas = this.sys.game.canvas;
            const width = canvas.width;
            ...
            let timeGap = Math.abs(
              orbStats.reduce((acc, cur) => acc.time - cur.time)
            );
            let text = `[color=red]${
              localeJSON["pTime"]
            } : [size=48][b]${orbStats[0].time.toFixed(
              2
            )}[/b][/size]s[/color][r][color=blue]${
              localeJSON["sTime"]
            } : [size=48][b]${orbStats[1].time.toFixed(
              2
            )}[/b][/size]s[/color][r][color=green]${
              localeJSON["timeGap"]
            } : [size=96][b][speed=100]${timeGap.toFixed(
              2
            )}[/speed][/b][/size]s[wait=1500]`;
...
class DigScene extends Phaser.Scene {
  ...
  create() {
    const canvas = this.sys.game.canvas;
    const width =
      Math.ceil((canvas.width * 1.5) / this.tileSize) * this.tileSize; //==可以移動範圍約1.5個螢幕寬
    const height =
      Math.ceil((canvas.height * 0.7) / this.tileSize) * this.tileSize;
    ...
    let initEnvironment = () => {
      let background = () => {
            ...
            this.tweens.add(
              Object.assign(
                ...
                animType == 1
                  ...
                  : {
                      tilePoanglesitionX: { start: 0, to: thing.width },
                      ease: "Linear",
                    }
              )
            );
        ...
        let underGround = () => {
          this.underBG = this.add
            .tileSprite(width * 0.5, this.groundY, 0, 0, "mineBG")
            .setDepth(0);
      ...
      let initChunks = () => {
        this.chunks = [];
        this.chunkSize = Math.ceil(
          Math.max(canvas.height, width) / this.tileSize
        );
        ...
        let seed =
          Math.abs(
            this.depthCounter.coordinate.reduce(
    ...
    let initPlayer = () => {
      this.player = this.add
        .existing(new Player(this))
        .setPosition(
          (parseInt((width / this.tileSize) * 0.5) - 0.5) * this.tileSize,
          0
        )
        .setDepth(Depth.player);
      ...
      Object.assign(this.player, {
        ...
        playerOpenGate: async () => {
          ...
          this.player.setVelocityX(0).play("player_idle", true);
          ...
          if (!this.sidekick.gateEventFlag) {
            ...
            for (let i = 0; i < Object.keys(lines).length; i++) {
              ...
              let dialogCode = await new Promise((resolve) =>
                this.RexUI.newDialog(line, { character, pageendEvent }, resolve)
              );
              // console.debug(dialogCode);
          ...
          let enterIdx = await new Promise((resolve) =>
            this.RexUI.newQuiz(questionData, 1, resolve)
          );
          ...
          if (questionData.options[enterIdx] == localeJSON.UI["yes"]) {
            ...
            //==SFX
            this.sound.play("sfx_levelClear");
          } else {
    ...
    let initSidekick = () => {
      let sidekick = () => {
        ...
        //===沒有震源提示,超過震源也提示
        const remindDepth =
          this.depthCounter.epicenter !== null
            ? this.depthCounter.epicenter +
              this.tileSize * this.depthCounter.depthScale
        ...
        Object.assign(this.sidekick, {
          ...
          remindingHadler: (sidekick, reminder = null) => {
            ...
            let callbackDelay =
              reminder != null || sidekick.firstTimeRemind ? 100 : remindDelay;
            let hint = "";

            //==挖超過幾公里開始提醒退出
            if (overDig) {
              hint =
                this.gameData.localeJSON.Hints["dig"][2][
                  this.depthCounter.epicenter === null ? 1 : 2
                ];
            ...
            this.remindingCallback = this.time.delayedCall(
              callbackDelay,
              () => {
    ...
    let initDepthCounter = () => {
      ...
      if (this.depthCounter.epicenter !== null) {
        let animsCreate = () => {
          this.anims.create({
            key: "bossDoor_shine",
            frames: this.anims.generateFrameNumbers("bossDoor", {
              frames: [0, 1, 2],
            }),
            frameRate: 5,
            repeat: -1,
  ...
  update() {
                ...
                switch (i) {
                  ...
                  case 2: //==1.深度
                    ...
                    guideArrow
                      ...
                      .setVisible(true)
                      .setPosition(
                        this.depthCounter.text.x + arrowWidth,
                        this.depthCounter.text.y
                      );
                    break;
                  case 3:
                    blackOut.scene.setVisible(false);
    ...
    let updatePlayer = () => {
      ...
      if (playerStats.MP < playerStats.maxMP)
        this.player.statsChangeHandler(
          { MP: playerStats.manaRegen / 100 },
          this
        ); //自然回魔(game update per 10ms,10ms=1/100s)
      if (playerStats.healthRegen > 0 && playerStats.HP < playerStats.maxHP)
        this.player.statsChangeHandler(
          { HP: playerStats.healthRegen / 100 },
          this
    ...
    let updateChunks = () => {
      ...
      for (let i = 0; i < this.chunks.length; i++) {
        ...
        let distBetweenChunks = Phaser.Math.Distance.Between(
          snappedChunkX,
          snappedChunkY,
          chunk.x,
          chunk.y
...
class BossScene extends Phaser.Scene {
  ...
  create() {
    ...
    let initEnvironment = () => {
      ...
      let flame = () => {
        ...
        let addFlame = (i, flameCount) => {
          this.add
            .sprite((width * i) / (flameCount + 1), height * 0.4, "bossFlame")
            .setScale(0.2)
            .setOrigin(0.5)
    ...
    let initPlayer = () => {
      ...
      this.player.attackEffect.setDepth(Depth.boss + 1);
        ...
        attackAnims: (resolve) => {
          ...
          this.tweens.add({
            ...
            onUpdate: (t) => {
              ...
              if (
                !this.player.attackingFlag &&
                t.elapsed > hurtDuration * 0.3
              ) {
                let effectShowDuration = hurtDuration * 0.2,
                  effectMoveDuration = hurtDuration * 0.6;

                this.player.play("player_attack").anims.setRepeat(-1);
                ...
                this.tweens.add({
                  ...
                  alpha: { start: 0, to: 1 },
                  onComplete: () =>
                    this.player.attackEffect.play("player_ultAttackEffect"),
                });
            ...
            onComplete: () => {
              this.player.play("player_idle").filpHandler(false);
    ...
    let initBoss = () => {
      ...
      //==落石發射器
      let particles = this.add.particles("bossRock").setDepth(Depth.boss + 1);
      ...
      let showAnims = () => {
        ...
        //==攻擊動畫
        let attackDelay =
          (flameCount + flyRepeat * (yoyoFlag ? 2 : 1)) * animeDelay;
        this.time.delayedCall(
          attackDelay,
    ...
    let initQuiz = () => {
      ...
      let getQuizIdxArr = (quizAmount) =>
        Array.from(new Array(quizAmount), (d, i) => i).sort(
          () => 0.5 - Math.random()
        );
      ...
      let showQuiz = async () => {
        let correct = await new Promise(
          (resolve) =>
            (this.quizObj = this.RexUI.newQuiz(getQuiz(), 0, resolve))
        );
        await new Promise((resolve) =>
          this[correct ? "player" : "boss"].attackAnims(resolve)
        );
    ...
    let initAudio = () => {
      let music = this.sound.add("bossFightTheme", {
        volume: 1,
        // rate: 1,
        // detune: 0,
        // seek: 0,
