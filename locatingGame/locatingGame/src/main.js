// const gameServer = "https://tecdc.earth.sinica.edu.tw/tecdc/Game/location/";
const gameServer = "https://localhost/Web/locatingGame/locatingGame/";
...
function locatingGame() {
  ...
  game.dataDir = async (value) => {
    ...
    let stationData = new Promise((resolve, reject) =>
      ajaxReadFile({ url: eventCatlog + "station.csv" }).then((success) => {
        ...
        data.map(async (d) => {
          d.waveData = Promise.all(
            channel.map(async (cha) => {
              let path = dir + "." + d.station + "." + cha + fileExtension;
              return {
                channel: cha,
                data: await readTextFile(path, fileDataKey),
              };
            })
          );
          return d;
    ...
    let tutorialData = new Promise((resolve, reject) => {
      ...
      const files = [
        "2010.166.TDCB.BHE.xy",
        "2010.166.TDCB.BHN.xy",
        "2010.166.TDCB.BHZ.xy",
      ];
      const fileDataKey = ["x", "y"];
      const tutorialData = {
        //09850, 16630
      ...
      data = Promise.all(
        files.map(async (file) => {
          let path = dir + file;
          return {
            channel: file[file.lastIndexOf(".") - 1],
            data: await readTextFile(path, fileDataKey),
          };
        })
      );
    ...
    rankingData = new Promise((resolve, reject) =>
      ajaxReadFile({ url: datafileDir + "rank/records.txt" }).then(
        (success) => {
          let data;
          data = success
            .split("\n")
    ...
    data = Promise.all([stationData, epicenterData, tutorialData]).then(
      (sucess) => {
  ...
  async function game() {
    ...
    function gameGenerate() {
      ...
      let getAudioPlayer = () => {
        let audioPlayer = new Audio(`${assetsDir}audio/BGM/quiz.wav`);
      ...
      let hintTextAnime = (description = "itemGain1", items = null) => {
              ...
              let imgs = items
                .map(
                  (item) =>
                    `<img src="${
                      itemsDir + item[0]
                    }.png" width="${imgW}px"  height="${imgW}px">`
                )
                .join("");
        ...
        let itemFlyToBag = (hintGroup, duration = 500) => {
          ...
          let itemAnime = (item) => {
            ...
            let goalPos = [
              bagPos.left - targetPos.left,
              bagPos.top - targetPos.top,
            ];
            // console.debug(targetPos, goalPos);
            item.animate(
              { left: goalPos[0], top: goalPos[1] },
              duration,
              "swing",
              () => bagAnime(item)
            );
          ...
          let bagAnime = (item) => {
            const duration = 250;
            const originW = bagImg.prop("width"),
              bigW = originW * 2,
          ...
          items.each((i, item) =>
            setTimeout(() => itemAnime($(item)), i * 500)
          );
        };
        let hintText = (hintGroup) => {
          ...
          let string = items
            ? `${
                ...
                GameData.localeJSON.UI["itemGain"]
              } 
                    ${items
                      .map(
                        (item) =>
                          GameData.localeJSON.Item[item[0]].name +
                          (item[1] ? `x${item[1]}` : "")
                      )
                      .join(", ")}`
            : GameData.localeJSON.UI[description];
          ...
          setTimeout(() => {
            hint.animate({ opacity: 0 }, fadeDuration, "swing", () =>
              hintGroup.remove()
            );
        ...
        if (items) {
          items.forEach((item) => {
            if (item[1] === 0) GameData.backpack.equip.push(item[0]);
            else {
              let itemName = item[0],
                itemAmount = item[1];
      ...
      function initEndScene(clear = false) {
        ...
        let congratsScene = async () => {
          ...
          let initRankChart = () => {
            ...
            congrats.find(".rankChart").append(svg).find("svg");
            // .height(height)
          ...
          let initMenu = () => {
            ...
            endMenu.find("#fbButton").on("click", () => {
              let getProfile = () => {
                const pictureW = 100;
                return new Promise((r) => {
                  FB.api(
                    "/me",
                    {
                      fields: `id,name,picture.width(${pictureW}).height(${pictureW}).redirect(true)`,
                    },
                    function (response) {
              ...
              let share = async (profilePromise = null) => {
                const certificateDir = gameServer + "certificate/";
            ...
            endMenu.find("#downloadButton").on("click", () => {
              getSharingImg();
              // console.debug(imgName);
            });
            endMenu.find("#surveyButton").on("click", () => {
              window.open(
                "https://docs.google.com/forms/d/e/1FAIpQLSfwCKL5kVIqeUxJD7QTBj0VcSonCJBrud2QGGfPfd_AQvZuRg/viewform?usp=sf_link"
              );
            });
            endMenu.find("#rankButton").on("click", () => {
              congrats.trigger("openPanel", "rank");
              gameOuterDiv.css("pointer-events", "all");
            });
            endMenu.find("#linkButton").on("click", () => {
      ...
      function initMap() {
        ...
        let requestTypingAnim = () => {
          ...
          typingText.removeClass("typingText").css({ visibility: "hidden" });

          window.requestAnimationFrame(function (time) {
            window.requestAnimationFrame(function (time) {
              typingText.addClass("typingText").css({ visibility: "visible" });
            });
          });
        };
        ...
        async function addStation() {
          ...
          const enemyArr = Object.keys(GameObjectStats.creature).filter(
            (c) => c != "boss" && c != "zombie"
          );
          ...
          data.forEach((d, i) => {
            ...
            let enemy =
              copyEnemyArr.length !== 0 //===拷貝的陣列抽完才全隨機
                ? [copyEnemyArr.pop()]
            ...
            let tooltipHtml = `
                            <text class='staName'>${
                              GameData.localeJSON.UI["station"] +
                              " : " +
                              d["station"]
                            }</text><br>
                            <text class='enmeyType'>${
                              GameData.localeJSON.UI["enmey"] + " : "
                            }</text>
                            ${enemy
                              .map(
                                (e) =>
                                  `<img src='${
                                    assetsDir + "icon/" + e + ".png"
                                  }' width='25px'></img>`
                              )
                              .join(" ")}<br>            
                        `;
        ...
        async function addUI() {
          ...
          const UItooltip = gameUI
            .append(
              `<div class="UItooltip"><div class="tooltipText"></div></div>`
            )
            .find(".UItooltip");
          ...
          const guideArrow = gameUI
            .append(
              `<div class="guideArrow"><img src="${assetsDir}ui/map/guideArrow.gif"></img></div>`
            )
            .find(".guideArrow");
          ...
          function updateTooltip(target) {
            ...
            let UI_index = UIbuttons.indexOf(target.id),
              hotKey =
                UI_index != -1
                  ? target.id === "backpack"
                    ? "I"
                    : UIbuttons[UI_index][0].toUpperCase()
                  : null;

            UItooltip.show() //==先show才能得到寬高
              .children(".tooltipText")
              .text(
                GameData.localeJSON.UI[target.id] +
                  (hotKey ? ` (${hotKey}) ` : "")
              );
            ...
            if (
              !GameData.stationClear.chartUnlock &&
              target.id == "velocityChart"
            ) {
              UIhint.animate({ opacity: "show" }, 500)
                .children(".tooltipText")
          ...
          function updateUI(target, show) {
            ...
            if (show) {
              ...
              let top =
                  id == "questInfo" || id == "velocityChart"
                    ? height * 0.1
                    : targetDOMRect.top - bigMapDOMRect.top,
              ...
              else if (id == "playerStats" || id == "backpack")
                //==人物圖更新
          ...
          let UIbar = () => {
            ...
            let addIcons = () => {
              ...
              let iconsHtml = UIbuttons.map(
                (btn, i) => `
                            <div class="UIicon" id="${btn}" style="top:${
                  interval * (i + 1) - iconW * 0.5
                }px; left:${left}px">
              ...
              UIbuttons.forEach((btn) => {
                ...
                switch (btn) {
                    ...
                    UI.find(".barBox")
                      .on("mouseover", function (e) {
                        ...
                        UI.find(".black-tooltip")
                          ...
                          .css({
                            top: `${
                              isHP ? 0 : this.getBoundingClientRect().height * 2
                            }px`,
                            left: `${e.offsetX}px`,
                          })
                          .show();
                    ...
                    UI.on("updateEvt", () => {
                      ...
                      if (onEquip.length !== 0)
                        onEquip.forEach((item) => {
                          ...
                          Object.keys(buffs).forEach(
                            (stat) => (playerStats[stat] += buffs[stat])
                          );
                        });
                      // console.debug(UI);
                      let hpPercent =
                  ...
                  case "velocityChart":
                    ...
                    UI.append(
                      `
                                            <div>
                                                <h2>${
                                                  GameData.localeJSON.UI[
                                                    "velocityStr"
                                                  ]
                                                }：
                                                    <div style="white-space:nowrap;text-align:center;">
                                                        <b id="velocityStr" style="color:Tomato;font-size:60px;">${GameData.velocity.toFixed(
                                                          2
                                                        )}</b> km/s
                                                    </div>
                                                </h2>
                    ...
                    UI.on("addDropdownItem", (e, data) => {
                      ...
                      dropdown
                        .append(html)
                        .find(`#display_${station}`)
                        .change(function () {
                          data.circleObj._path.style.display = this.checked
                            ? "inline"
                            : "none";
                        });

                      UI.find(".dropdown-menu").on("click.bs.dropdown", (e) =>
                        e.stopPropagation()
                      );
                    });
                  ...
                  case "questInfo":
                    ...
                    let getImgHTML = (img, imgScale = 1) => {
                      return `<img src='${infoImgDir + img}' width='${
                        imgW * imgScale
                      }px'></img>`;
                    };
                    ...
                    let infoListHTML = [...Array(infoCount).keys()].map(
                      (d, i) => `
                                            <a class="list-group-item list-group-item-action ${
                                              i === 0 ? "active" : ""
                                            }" data-toggle="list" href="#questInfo${i}" role="tab">${
                        GameData.localeJSON.UI["questInfo" + i]
                      }</a>
                                        `
                    );
                    let infoContentHTML = [...Array(infoCount).keys()].map(
                      (d, i) => {
                        let content =
                          GameData.localeJSON.Intro["questInfo" + i];
                        ...
                        switch (i) {
                          ...
                          case 2:
                            content = content
                              .replace("\t", getImgHTML("2.png", 0.8))
                              .replace(
                                "[link]",
                                `<a target="_blank" href="https://www.cwb.gov.tw/Data/service/Newsbb/CH/1081218earthquakepress.pdf">`
                              )
                              .replace("[/link]", "</a>");
                            break;
                          case 5:
                            content = content.replace(
                              "\t",
                              getImgHTML("3.png", 0.8)
                            );
                            break;
                          case 6:
                            content = content.replace(
                              "\t",
                              getImgHTML("4.png", 0.9)
                            );
                            break;
                          case 7:
                            content = content
                              .replace("\t", getImgHTML("5.png"))
                              .replace("\t", getImgHTML("6.png", 0.8));
                            break;
                          case 8:
                            content = content.replace(
                              "\t",
                              getImgHTML("7.png", 0.8)
                            );
                            break;
                        }
                        ...
                        return `<div class="tab-pane fade ${
                          i === 0 ? "show active" : ""
                        }" id="questInfo${i}" role="tabpanel" style="white-space: pre-line">
                                                <div>${content}</div>
                                                <div class="d-flex justify-content-end questFooter">
                                                    <button type="button" class="btn btn-primary questButton">${
                                                      GameData.localeJSON.UI[
                                                        "quickAnswer"
                                                      ]
                                                    }</button>
                    ...
                    let questButton = UI.find(".questButton").each(
                      (i, button) =>
                        $(button).on("click", () => {
                          if (quickQuestion.css("display"))
                            quickQuestion.hide();

                          let questionObj =
                              GameData.localeJSON.quickQuestion[i],
                            choices = [...questionObj.choices].sort(
                              () => 0.5 - Math.random()
                            );

                          quickTitle.text(
                            "Q" + (i + 1) + ": " + questionObj.content
                          );
                    ...
                    UI.find(".quickOK").on("click", (e) => {
                      ...
                      if (isAnswer == undefined) {
                        ...
                        if (isAnswer === "true") {
                          ...
                          let getRandomAmount = (min = 3) => {
                            //==隨機最小幾個到+2
                            return getRandom(3) + min;
                          };
                          let getRandomItem = (type = 0) => {
                            let items = Object.keys(GameItemData).filter(
                              (item) => GameItemData[item].type === type
                            );
                          ...
                          let gainItems =
                            bingoCount % 3 === 0
                              ? [
                                  ...
                                  [getRandomItem(), getRandomAmount()],
                                  ["seeds", getRandomAmount()],
                                ]
                              : bingoCount % 3 === 1
                              ? [
                                  [getRandomItem(), getRandomAmount()],
                                  ["bone", getRandomAmount()],
                                ]
                              : [
                                  [getRandomItem(), getRandomAmount()],
                                  ["catfood", getRandomAmount()],
                                ];
                  ...
                  case "backpack":
                    ...
                    UI.width(510).height(320).append(`
                                            <div class='black-tooltip'></div>
                                            <div class='row'>
                                                <div class='col-5'>
                    ...
                    let getItemImg = (idx, key) => {
                      ...
                      let imgHtml = item
                        ? `<img class="item" src="${
                            itemsDir + (key === "item" ? item.name : item)
                          }.png" width="${iconW}px" height="${iconW}px" value="${idx}">
                                                ${
                                                  key === "item"
                    ...
                    let getTableHTML = (key) => {
                      ...
                      if (key === "item") {
                        ...
                        blockSize[key] =
                          itemLength > blockSize[key].reduce((p, c) => p * c)
                            ? [Math.ceil(itemLength / 4), 4]
                            : blockSize[key];
                      }
                    ...
                    let updateTooltip = (target, blockKey) => {
                      ...
                      let blockText = {
                        ...
                        itemBuff: Object.keys(itemBuff)
                          .map(
                            (key) =>
                              `${
                                GameData.localeJSON.UI[key] +
                                (itemBuff[key] > 0 ? " +" : " ") +
                                itemBuff[key]
                              }`
                          )
                      ...
                      let html = tooltipBlock
                        .map(
                          (key) =>
                            `<div class="tooltipText" id="${key}">${blockText[key]}</div>`
                        )
                        .join("");
                    ...
                    let updateSelectBar = (target, blockKey) => {
                      ...
                      let html = selection
                        .map(
                          (key) =>
                            `<div class="" option="${key}">${GameData.localeJSON.UI[key]}</div>`
                        )
                        .join("");
                    ...
                    itemSelectBar.on("click", (e) => {
                      ...
                      switch (option) {
                        ...
                        case "equip":
                          ...
                          item =
                            GameData.backpack[selectData.key][selectData.idx];
                          ...
                          if (gameItemData.type === 0) {
                            Object.keys(gameItemData.buff).forEach((key) => {
                              GameData.playerStats[key] +=
                                gameItemData.buff[key];
                              if (key === "HP" || key === "MP")
                                GameData.playerStats[key] =
                                  GameData.playerStats[key] >
                                  GameData.playerStats["max" + key]
                                    ? GameData.playerStats["max" + key]
                                    : GameData.playerStats[key];
                            });
                            //==數量用完就刪除
                            if ((item.amount -= 1) <= 0)
                              GameData.backpack[selectData.key].splice(
                                selectData.idx,
                                1
                              );
                          }
                        ...
                        UI.find(".black-tooltip")
                          ...
                          .css({
                            top: `${
                              ...
                              (isHP
                                ? 0
                                : this.getBoundingClientRect().height * 1.5)
                            }px`,
                            left: `${e.offsetX}px`,
                          })
                          .show();
                ...
                let closeBtn = UI.append(`<div class="closeBtn">✖</div>`).find(
                  `.closeBtn`
                );
            ...
            let iconEvent = () => {
              ...
              gameUI
                ...
                .on("mouseout", (e) => {
                  ...
                  if (
                    !GameData.stationClear.chartUnlock &&
                    e.target.parentNode.id == "velocityChart"
                  )
                    UIhint.hide();
                })
                .on("click", function (e) {
                  ...
                  if (
                    this.id == "velocityChart" &&
                    GameData.sidekick.lineStage[0] == 3
                  ) {
          ...
          let confirmWindow = () => {
            ...
            confirmWindow
              ...
              .on("click", (e) => {
                ...
                if (yes) {
                  let gameStartParameters = placeStr.data(
                    "gameStartParameters"
                  );
          ...
          let sidekick = () => {
            ...
            let init = () => {
              ...
              sidekickUI
                ...
                .hide()
                .on("click", () =>
                  GameData.sidekick.stopHotkey
                    ? false
                    : sidekick.trigger("click")
                );
            };
            init();
            updateSidekick(...GameData.sidekick.lineStage);
        ...
        function addMapEvent() {
          ...
          mapObj
            .on("click", function (e) {
              ...
              let distToEpicenter = distanceByLnglat(
                [lat, lng],
                data.epicenter.coordinate
              );
              // DEV:顯示震央偏離點擊距離
              ...
              assumedEpicenter
                ...
                .setContent(
                  `${
                    GameData.localeJSON.UI["assumedEpicenter"]
                  } : ${coordinate.join(" , ")}`
                );
              ...
              if (gameUI.find("#velocityChartUI").is(":visible"))
                d3.select(`#velocityChartUI>svg`).dispatch("updateEvt");
              else gameUI.find("#velocityChart").trigger("click");
            })
            .on("move", function (e) {
          ...
          $(window).on("keyup", (e) => {
            ...
            switch (e.code) {
              ...
              case "KeyN":
                ...
                confirmWindow
                  .find(`text[value = "${e.code == "KeyY" ? "yes" : "no"}"]`)
                  .trigger("click");
                break;
            }
          });
      ...
      function updateStation(stationMarker, updateObj = {}) {
        ...
        let iconTriggerAnime = (marker, trigger = true, duration = 50) => {
          if (interval) return;
          const iconScale = 1.5;
          const iconSize1 = trigger
              ? originalIconSize
              : originalIconSize * iconScale,
            iconSize2 = trigger
              ? originalIconSize * iconScale
              : originalIconSize;
        ...
        let iconBrokenAnime = (marker, duration = 500) => {
          const iconUrl1 = marker.getIcon().options.iconUrl,
          ...
          if (updateObj.hasOwnProperty("stationInfo")) {
            ...
            if (updateObj.stationInfo.clear) {
              ...
              if (!stationStats.clear) {
                ...
                gameUI
                  .find(`#velocityChartUI`)
                  .trigger("addDropdownItem", data);
              }
              ...
              if (orbStats) {
                ...
                let tooltipContent = `
                                    ...
                                    <text class='pTime'>${
                                      GameData.localeJSON.UI["pTime"] +
                                      " : " +
                                      parseFloat(orbStats[0].time.toFixed(2)) +
                                      " s"
                                    }</text><br>
                                    <text class='sTime'>${
                                      GameData.localeJSON.UI["sTime"] +
                                      " : " +
                                      parseFloat(orbStats[1].time.toFixed(2)) +
                                      " s"
                                    }</text><br>
                                    <text class='timeGap'>${
                                      GameData.localeJSON.UI["timeGap"] +
                                      " : " +
                                      parseFloat(timeGap.toFixed(2)) +
                                      " s"
                                    }</text><br>
                                    <text class='preDistance'>${
                                      GameData.localeJSON.UI["preDistance"] +
                                      " : " +
                                      parseFloat((radius / 1000).toFixed(1)) +
                                      " km"
                                    }</text><br>
                                    ...
                                    ${Object.keys(stationStats.enemyStats)
                                      .map((e) =>
      ...
      function updateMapUI(gameResult, duration = 600) {
        ...
        if (pre_timeRemain > timeRemain)
          GameData.playerTimeUse += pre_timeRemain - timeRemain;

        const start = pre_timeRemain * GameData.timeMultiplier,
        ...
        let timerAnime = (increase) => {
          ...
          let interval = setInterval(() => {
            ...
            timerTexts[2].innerHTML = parseInt(
              (notEnoughForDays % 3600000) / 60000
            );
        ...
        let stageUnlock = () => {
          if (!GameData.stationClear.chartUnlock) {
            GameData.stationClear.count = data.filter(
              (d) => d.stationStats.clear
            ).length;
      ...
      function updateSidekick(stage, index, doneTalking = undefined) {
        ...
        let showText = (stage, index, doneTalking) => {
          ...
          if (!GameData.sidekick.doneTalking || doneTalking == false) {
            ...
            switch (`${stage}_${index}`) {
              ...
              case "2_0":
                replaceHTML = `<text>${
                  clearStationToUnlock - GameData.stationClear.count
                }</text>`;
                line = line.replace("\t", replaceHTML);
                break;
              case "3_1":
            ...
            GameData.sidekick.doneTalking =
              doneTalking == undefined ? index == endIdx : doneTalking;
        ...
        // console.debug(stage, index, doneTalking);
        if (!audioObj.flag && !(stage === 1 && index === 0)) {
          // console.debug("audio start");
          audioObj.flag = true;
      ...
      async function gameStart(gameMode, siteData = null) {
        ...
        if (gameMode == "defend") {
          ...
          // DEV: 3.直接進王關 gameResult.bossRoom=1
          {
          ...
          // gameResult = { bossRoom: 1 };
          //===進王關
          if (gameResult.bossRoom) {
  ...
  async function getWaveImg(
    ...
    let waveData = await (stationData.waveData
      ? stationData.waveData
      : data.tutorialData);
    // console.debug(waveData);

    function getSvgUrl(data) {
      //==max min要一樣起始點才會落在同位置(避免波形間隔看起來不同)
      const xAxisDomain = timeDomain
        ? timeDomain
        : d3.extent(data[0].data.map((d) => d.x));
      const yAxisDomain = d3.extent(
        [].concat(...data.map((d) => d3.extent(d.data, (d) => d.y)))
      );
            ...
            g
              .attr(
                ...
                `translate(0,${
                  height -
                  margin.bottom *
                    (stationData.isTutorial && !overview ? 2.5 : 1)
                })`
              )
          ...
          let makePaths = (pathGroup) =>
            pathGroup
              ...
              .attr(
                ...
                (d, i) =>
                  `translate(0,${
                    eachHeight * i + margin.top + getShiftY(d.data[0].y)
                  })`
              )
              .call((gCollection) =>
                gCollection.each(function (d, i) {
                  ...
                  g.append("text")
                    .attr(
                      "transform",
                      `translate(${margin.left},${y(data[0].y)})`
                    )
                    .attr("fill", color)
                    ...
                    .attr("font-size", "20")
                    .text(
                      channel + GameData.localeJSON.UI["channel" + channel]
                    );
                })
              );
  ...
  function getVelocityChart() {
    ...
    function getNewData() {
      epicenterCoord = GameData.playerEpicenter
        ? GameData.playerEpicenter
        : epicenterCoord;
        ...
        .map(
          (d) =>
            new Object({
              ...
              timeGap: Math.abs(
                d.stationStats.orbStats.reduce(
                  (acc, cur) => acc.time - cur.time
                )
              ),
              // timeGap: 10,
              data: d,
    ...
    function updateChart(handleUpdate = false, trans = true) {
      ...
      function render() {
        ...
        let xAxisDomain = [
          ...
          newDataObj.length == 0
            ? 10
            : d3.max(newDataObj.map((d) => d.timeGap)) * domainScale,
        ];
        let yAxisDomain = [
          0,
          d3.max(
            data.map((d) => distanceByLnglat(d.coordinate, epicenterCoord))
          ) * domainScale,
        ];
        ...
        let updateAxis = () => {
          let makeXAxis = (g) =>
            g
              ...
              .call((g) =>
                g.select(".domain").attr("stroke-width", strokeWidth)
              );
                ...
                focusGroup.selectAll(".dots").each(function (d, i) {
                  ...
                  if (trans)
                    dot
                      ...
                      .interrupt()
                      .transition()
                      .duration(trans ? transDuration : 0) //.interrupt()前次動畫
          ...
          let makeArcArea = (fixedGroup) =>
            fixedGroup
              ...
              .call((g) => {
                ...
                let start =
                  Math.PI / 2 + Math.asin((rangePoint[0].y - y.range()[0]) / r);
                let end =
                  Math.PI / 2 + Math.asin((rangePoint[1].y - y.range()[0]) / r);
    ...
    function events(svg) {
      ...
      let handleDrag = () => {
        ...
        let dragBehavior = d3.drag().on("drag end", function (e) {
          // console.log('drag');
          // console.debug(e.x, e.y);
          let slope = y.invert(e.y) / x.invert(e.x);

          if (slope < slopeRange[0]) slope = slopeRange[0];
          else if (slope > slopeRange[1]) slope = slopeRange[1];
      ...
      let focusHover = () => {
        ...
        const tooltip = UI.append("div").attr("class", "black-tooltip");
  ...
  function getRankChart(rankingData) {
    ...
    function getNewData() {
      ...
      let getGapGroupData = () => {
        ...
        rankingData.forEach((d) => {
          ...
          if (d === playerObj)
            gapGroupData.playerGroupIdx = gapGroupData.length - groupIndex - 1;
      ...
      let getPR = () => {
        ...
        return parseFloat(
          (((playerIdx + 1) / rankingData.length) * 100).toFixed(1)
        );
      };
      let writeFile = () => {
        $.ajax({
    ...
    function updateChart(replay = false) {
      ...
      function init() {
        //==svg轉圖片.css不會套用所以寫在這裏
        svg.style(
          "text-shadow",
          "#111 0 0 2px, rgba(255, 255, 255, 0.1) 0 1px 3px"
        );
      ...
      function render() {
          ...
          line = d3
            ...
            .x((d, i) => {
              let multi =
                gapGroupData.length -
                i -
                (i == 0 ? 2 : i == gapGroupData.length - 1 ? 1 : 1.5);
              return x(multi * gap);
            })
            .y((d) => y(d));
        };
          ...
          let makeXAxis = (g) =>
            g
              ...
              .call((g) => {
                ...
                axisFun.tickValues(tickValues).tickFormat((d) => d); //==不然小數會被轉整數
                axisFun(g);
              })
              .call(setStyle)
              .call((g) =>
          ...
          let makeYAxis = (g) =>
            g
              ...
              .call((g) => {
                ...
                let tickValues = y.ticks().filter(Number.isInteger);
        ...
        let updateFocus = () => {
          ...
          bar
            ...
            .interrupt()
            .transition()
            .duration(appearDuration) //.interrupt()前次動畫
          ...
          bar
            ...
            .delay(
              (d, i) =>
                appearDuration + i * (runDuration / (playerGroupIdx + 1))
            )
            .ease(d3.easeLinear)
            .call((bar) =>
              bar.filter((d, i) => i == playerGroupIdx).attr("fill", barFill2)
            )
        ...
        let playAnime = (replay) => {
          ...
          let init = () => {
            brave.attr("href", braveDir + `${playerRole}_Run.gif`);
          ...
          let walkingAnime = () => {
            ...
            braveAnim
              ...
              .attr("path", () =>
                line(gapGroupData.slice(0, playerGroupIdx + 2))
              )
              .attr("repeatCount", "1")
              .attr("fill", "freeze")
              .attr("rotate", 0);
              ...
              .on("endEvent", () => {
                ...
                d3.timeout(() => {
                  ...
                  boss
                    ...
                    .attr(
                      "transform-origin",
                      `${bossX + 0.5 * bossW} ${bossY + 0.3 * bossW}`
                    )
                    .attr("transform", `scale(-1,1) rotate(-180)`)
                    .transition()
                    .duration(bossT1) //.interrupt()前次動畫
                    .ease(d3.easeSinOut)
                    .attr("y", bossY + 15)
                    .transition()
                    .duration(bossT3)
      ...
      let barHover = () => {
        let updateTooltip = (groupIndex, data) => {
          tooltip
            ...
            .attr(
              "class",
              "d-flex flex-nowrap justify-content-center align-items-end text-nowrap"
            )
            .call((divC) =>
              divC.each(function (d, i) {
                ...
                if (i == 0) {
                  ...
                  html =
                    (d == 0
                      ...
                      : `${range1}&nbsp;~&nbsp;${range2}`) +
                    "&nbsp;<h5>pt</h5>";
                } else html = `<h1 style='font-weight:bold;'>${d}</h1>&nbsp;<h6>${GameData.localeJSON.UI["playerCount"]}</h6> `;
        ...
        braveBars
          ...
          .on("mouseout", (e) => {
            ...
            braveBars.attr("opacity", (d, i) =>
              i <= playerGroupIdx ? afterOpacity : originOpacity
            );
  ...
  function getSharingImg(profile = null) {
    ...
    const svg = d3
      .create("svg") // const svg = d3.select(document.createElement("svg"))
      ...
      function init() {
        ...
        focusGroup.call((g) => {
          g.append("text")
            ...
            .attr("y", height * 0.3)
            .text(
              `${localeJSON["certLabel1"]} ： ${
                profile ? profile.name : GameData.playerCustom.name
              }`
            );
          ...
          let timeUse = {
            ...
            timeUseStr =
              (timeUse.hour > 0
                ? timeUse.hour + " " + localeJSON["HRS"] + " "
                : "") +
              (timeUse.hour > 0 || timeUse.min > 0
                ? timeUse.min + " " + localeJSON["MINS"] + " "
                : "") +
              timeUse.sec +
              " " +
              localeJSON["SECS"];
          ...
          g.append("text")
            ...
            .text(
              `${localeJSON["certLabel3"]} ： ${new Date()
                .toISOString()
                .substring(0, 10)}`
            );
          ...
          g.append("text")
            ...
            .text(`${localeJSON["certLabel4"]} ： ${gameServer}`);
    ...
    async function getCertRes() {
      ...
      let imgResource = {
        ...
        photo: profile
          ...
          : await composeCertificate(null, null, "avatar"),
    ...
    let fileName =
      GameData.playerCustom.name +
      GameData.localeJSON.UI["whosCert"] +
      "_" +
