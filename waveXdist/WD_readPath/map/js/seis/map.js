...
function addLayerToMap(data, circleSize, isTesis = false) {
    ...
    for (let i in data) {
        ...
        marker = L.circleMarker([row.Latitude, row.Longitude], { radius: size, color: colorcode }
            ...
            "<br>Depth: " + row.Depth + " km <br>ML: " + row.ML + "<br>" + `<button type="button" class="btn btn-secondary" onClick="selectCatalog('${row.catlog}')">Show waveform</button>`
        );
...
function get_events() {
    ...
    axios.post('../data/eventlist_all.txt')
        .then(response => {
            $('#load').hide();
