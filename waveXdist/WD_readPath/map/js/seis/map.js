...
$(document).ready(function () {
  Map.on("baselayerchange", function (layer) {
    ...
    if (layer.name === "WorldImagery") {
      $(
        "#depscale text,#MLscale text:first-of-type,#MLscale text:last-of-type"
      ).css({
        stroke: "white",
        "stroke-width": 0.6,
      });
...
let s100Color = ["#DB453F", "#F2B53A", "#97E838", "#26D2EB", "#7B3AF2"];
function getColor(depth) {
  ...
  //   let colorIdx = dep.findIndex((d) => depth / d <= 1);
  let colorIdx = dep.findIndex((d) => depth <= d);
  return s100Color.slice(colorIdx)[0];
  //   return s100Color[colorIdx < 0 ? s100Color.length - 1 : colorIdx];
}

function getSize(ML, circleSize) {
  let ml_base = 3;
  if (ML > ml_base) {
    return (ML - ml_base) * circleSize + 0.1;
  } else {
    return 0.1;
  }
}
  ...
  for (let i in data) {
    ...
    marker = L.circleMarker([row.Latitude, row.Longitude], {
      ...
      row.Date +
        ...
        row.ML +
        "<br>" +
        `<button type="button" class="btn btn-secondary" onClick="selectCatalog('${row.catlog}')">Show waveform</button>`
    );
...
function get_events() {
  ...
  axios
    .post("../data/eventlist_all.txt")
    .then((response) => {
      $("#load").hide();
      // console.debug(response.data);
      eqData = response.data.split("\n").map((row) => {
        let col = row.split(/\s+/);
        // console.debug(col);
        return {
          Date: col[0],
