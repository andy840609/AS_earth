const Bullet = new Phaser.Class({
  ...
  fire: function (attacker, attackSpeed, attackRange = false, onXaxis = true) {
    ...
    if (attacker.stats.class != 0)
      this[onXaxis ? "setVelocityX" : "setAccelerationY"](
        attackSpeed * fireDir
      ); //==不同軸向的攻擊
  ...
  update: function (time, delta) {
    ...
    if (this.attacker.stats.class == 0) {
      ...
      this.setVelocityX(this.attacker.body.velocity.x).setVelocityY(
        this.attacker.body.velocity.y
      );
      ...
      let outOfRange = this.attackRange
        ? Phaser.Math.Distance.BetweenPoints(this.attacker, this) >
          this.attackRange
        : false;
      let outOfWindow = !this.scene.cameras.main.worldView.contains(
        this.x,
        this.y
      );
...
const Enemy = new Phaser.Class({
  ...
  initialize: function Enemy(scene, key, i, stats) {
    ...
    this.body.setSize(...bodySize).setGravityY(5000);
    ...
    let EnemyBehaviorFunction = {
      dog: () => {
        ...
        if (!this.stats.active)
          if (
            Phaser.Math.Distance.BetweenPoints(player, this) < 300 ||
            this.behavior == "hurt" ||
            this.behavior == "possessed"
          )
          ...
          switch (this.behavior) {
            case "hurt":
              ...
              if (!this.knockBackCallback)
                this.knockBackCallback = scene.time.delayedCall(
                  knockBackDuration,
                  () => {
            ...
            case "barking":
              ...
              if (!this.behaviorCallback) {
                ...
                this.behaviorCallback = scene.time.delayedCall(
                  barkingDuration,
                  () => {
                    this.behavior = "chasing";
                    this.behaviorCallback = null;
            ...
            case "chasing":
              ...
              //==chasing
                ...
                scene.physics.accelerateToObject(
                  this,
                  player,
                  speed,
                  speed * 1.1
            ...
            case "possessed":
              ...
              let boneIdx = scene.itemOnFloor.findIndex(
                (item) => item.name === "bone"
              );
              ...
              if (boneIdx !== -1) {
                ...
                if (Phaser.Math.Distance.BetweenPoints(this, target) < 40) {
                  ...
                  target.destroyCallback = scene.time.delayedCall(
                    destroyDur,
                    () => {
                      target.destroy();
      ...
      cat: () => {
        //===看完教學啟動?
        if (!this.stats.active) this.stats.active = true;
        //===開始行爲模式(0.受傷 1.追擊(被攻擊時) 2.遊走 3.休息 )
          ...
          switch (this.behavior) {
                ...
                if (this.body.touching.down) {
                  ...
                  if (
                    ...
                    this.body.setVelocity(
                      speed * (player.x > this.x ? 1 : -1),
                      -this.stats.jumpingPower
                    );
                  }
                  //==以加速度追
                  else {
                  ...
                  if (this.flipX != filpDir) this.filpHandler(filpDir);
            ...
            case "cruising":
              ...
              if (!this.behaviorCallback) {
                ...
                let randomX = Phaser.Math.Between(
                  0,
                  scene.sys.game.canvas.width - 16
                ); //==隨機移動到螢幕內x;
                let speed = this.stats.movementSpeed; //pixel per sec
                let dist = Phaser.Math.Distance.BetweenPoints(this, {
                  x: randomX,
                  y: this.y,
                });
            ...
            case "possessed":
              ...
              let catfoodIdx = scene.itemOnFloor.findIndex(
                (item) => item.name === "catfood"
              );
              // console.debug(boneArr)
              if (catfoodIdx !== -1) {
        ...
        //===開始行爲模式(0.受傷 1.攻擊 2.追擊 3.休息 )
          ...
          switch (this.behavior) {
            ...
            case "flying":
              ...
              if (!this.behaviorCallback) {
                ...
                let randomY = Phaser.Math.Between(
                  50,
                  scene.sys.game.canvas.height * 0.3
                );
                let speed = this.stats.movementSpeed; //pixel per sec

                scene.physics.accelerateTo(
                  this,
                  randomX,
                  randomY,
                  speed / 2,
                ...
                if (dist <= 5) {
                  ...
                  this.bulletCallback = scene.time.delayedCall(
            ...
            case "worship":
              ...
              if (this.behaviorCallback) {
                if (
                  Object.prototype.toString.call(this.behaviorCallback) ===
                  "[object Object]"
                )
                  this.behaviorCallback.remove();
                this.behaviorCallback = null;
              ...
              scene.physics.accelerateTo(
                this,
                this.x,
                scene.platforms.getChildren()[0].y,
                speed / 2,
                speed
              );
            ...
            case "possessed":
              ...
              let seedsIdx = scene.itemOnFloor.findIndex(
              ...
              if (seedsIdx !== -1) {
                ...
                // console.debug()
                if (
                  Phaser.Math.Distance.BetweenPoints(this, target) < 40 &&
...
const Player = new Phaser.Class({
  ...
  initialize: function Player(scene) {
    ...
    let animsCreate = () => {
      ...
      scene.anims.create({
        key: "player_crouch",
        frames: scene.anims.generateFrameNumbers("player_crouch"),
        frameRate: frameRate.crouch,
        repeat: 0,
      });
    ...
    //===init attack
    this.bullets = scene.physics.add
      .group({
        classType: Bullet,
    ...
    //====init equip
    this.equip = scene.add
      .image(0, 0, "onEquip_" + onEquip[0])
      .setPosition(-100, -100)
      .setOrigin(0.5)
      .setVisible(onEquip.length !== 0)
      .setDepth(scene.Depth.player + 1);
    if (onEquip.length !== 0)
      this.equip.setScale((this.displayWidth * 1.5) / this.equip.width);
  ...
  filpHandler: function (filp) {
    ...
    this.scene.time.delayedCall(
      this.dust.anims.isPlaying ? 500 : 0,
      () => {
  ...
  movingHadler: function (scene) {
    ...
    let isBusy =
      ((currentAnims === "player_runAttack" ||
        (currentAnims === "player_jumpAttack" && !this.body.touching.down)) &&
  ...
  pickingHadler: function (scene) {
    ...
    if (Phaser.Input.Keyboard.JustDown(cursors[controllCursor["down"]])) {
      if (this.pickUpObj) {
        ...
        //==光球特殊參數，非光球則為非
        let orbParameter =
          this.pickUpObj.name === "orb"
            ? this.pickUpObj.orbStats.isInRange
            : false;
        ...
        let pickUpList = (function () {
          //==可撿取清單
          let list = [];
          if (scene.player.emergFlag) {
            //==去掉不能撿的
            list = scene.doctor.emergItems
              .getChildren()
              .filter(
                (item) => item.name !== "emergTable" && item.name !== "emergBag"
              );
          } else {
  ...
  attackHandler: function (scene) {
    ...
    if (cursors[controllCursor["attack"]].isDown) {
      ...
      let isRuning =
        currentAnims === "player_run" || currentAnims === "player_runAttack";
  ...
  changeHPHandler: function (scene, hpChange) {
    ...
    if (!isIncrease) {
      //==SFX
      scene.sound.play("sfx_playerHurt");
      this.anims.play("player_hurt", true);

      //==取消攻擊動畫
      let ATK_anims = this.attackEffect.anims;
      if (ATK_anims.isPlaying)
        ATK_anims.setCurrentFrame(
          ATK_anims.currentAnim.frames[ATK_anims.currentAnim.frames.length - 1]
        );
  ...
  statsChangeHandler: function (statsObj) {
    Object.keys(statsObj).forEach((stat) => {
      ...
      //==加攻擊力等buff要加
      if (Object.keys(this.stats.buff).includes(stat))
        this.stats.buff[stat] += changeVal;
      ...
      if (stat == "HP" || stat == "MP") {
        ...
        if (stat == "HP") {
          if (this.stats.HP > 0) this.changeHPHandler(this.scene, changeVal);
          //==死
          else {
            ...
            //==SFX
            this.scene.sound.play("sfx_bossAttack");
...
const Sidekick = new Phaser.Class({
  ...
  initialize: function Sidekick(scene, key) {
    ...
    this.body.setSize(18, 26, true).setOffset(5, 6).setGravityY(200);
  ...
  talkingHandler: function (scene, hint) {
    ...
    this.talkingCallback = scene.time.delayedCall(
      hintDelay,
      () => {
        //==開始打字
        this.talkingTween = scene.tweens.add({
  ...
  behaviorHandler: function (player, scene) {
    ...
    switch (this.behavior) {
      ...
      case "following":
        if (this.body.touching.down) {
          ...
          if (
            !player.body.touching.down &&
            Phaser.Math.Distance.BetweenPoints(player, this) < 200
          ) {
            // console.debug(this.body.speed);
          ...
          //==以加速度追
            ...
            this.dust.setAlpha(1).play("sidekick_runDust", true);
          ...
          if (Phaser.Math.Distance.BetweenPoints(player, this) > 1000) {
            ...
            this.talkingHandler(
              scene,
              scene.gameData.localeJSON.Hints["dig"][2][0]
            );
          }
        }
    ...
    let getHint = () => {
      ...
      let hintType = this.dialog.preHintType ? 0 : Phaser.Math.Between(0, 1), //==0:提示 1:閒聊 2:特殊對話
      ...
      switch (scene.name) {
        case "defend":
          ...
          pickUpKey = scene.gameData.localeJSON.UI[pickUpKey]
            ? scene.gameData.localeJSON.UI[pickUpKey]
            : pickUpKey;
          //==有些提示不合時機就不出現
        ...
        case "tutorial":
          if (!hintType && !hintIdx)
            hint = this.hints[hintType][hintIdx].replace(
              replaceStr,
              scene.gameData.localeJSON.UI[this.name]
            );
          else hint = this.hints[hintType][hintIdx];
          break;
      }
...
const Doctor = new Phaser.Class({
  ...
  initialize: function Doctor(scene, lines) {
    Phaser.GameObjects.Sprite.call(this, scene, 0, 0, "doctorOwl");

    this.setScale(0.8).setOrigin(0).setAlpha(0).setName("doctor");
    ...
    this.emerDone = Array.from(new Array(this.emerAmount), () => 0); //==每種緊急每關只出一次
  },
  talkingTween: null,
  talkingCallback: null,
  behavior: null,
  behaviorHandler: function (player, scene) {
    ...
    this.dialog.setPosition(
      this.x + this.displayWidth * 1.9,
      this.y + this.displayHeight * 0.5
    );

    if (!this.talkingCallback) {
      let allEmerDone = this.emerDone.every((bool) => bool);
      //每次對話出現有6分之一觸發特殊事件
      let inEmergency =
        !allEmerDone &&
        player.scene.name === "defend" &&
        Phaser.Math.Between(1, 6) === 1;
      //   console.debug(allEmerDone);

      // console.debug(player.scene.name === 'defend',)
      // let inEmergency = true;
      if (!inEmergency) {
        const tipIdx = Phaser.Math.Between(0, this.tipAmount - 1), //==tip index
          tipText = this.tips[tipIdx];

        const tipDuration = tipText.length * 300, //==對話框持續時間(包含淡入淡出時間)一個字x秒
        ...
        const eventIdx = !isNaN(this.lastEventIdx)
            ? Phaser.Math.RND.pick(
                [...Array(this.emerAmount).keys()].filter(
                  (i) => i !== this.lastEventIdx
                )
              )
        ...
        this.emerDone[eventIdx] = 1; //==緊急完成不再出現

        const eventDelay = 2000, //事件開始3000
        ...
        let showTitle = (rules) => {
          ...
          downKey = gameData.localeJSON.UI[downKey]
            ? gameData.localeJSON.UI[downKey]
            : downKey;
        ...
        let dropPlayerItem = () => {
          ...
          if (player.pickUpObj) {
            ...
            //==光球特殊參數，非光球則為非
            let orbParameter =
              player.pickUpObj.name === "orb"
                ? player.pickUpObj.orbStats.isInRange
                : false;
        ...
        this.once("emergEnd", async function () {
          let destroyEventObj = () => {
            ...
            this.behavior = null;
            this.dialog.getElement("background").setStrokeStyle(2, 0xffffff, 1);
          ...
          if (eventClear) {
            ...
            let itemList = Object.keys(GameItemData) //==消耗品清單
              .filter(
                (item) =>
                  GameItemData[item].type === 0 || GameItemData[item].type === 1
              );
            ...
            [...Array(itemAmount).keys()].forEach((i) => {
              ...
              scene.time.delayedCall(i * 650, () =>
                //==掉落間隔時間
                scene.add.existing(
                  new Item(gameScene, itemList[itemIdx], ...itemPosition, true)
                )
              );
            });
          ...
          scene.tweens.add({
            ...
            ease: "Linear",
            onStart: () =>
              this.talkingTween ? this.talkingTween.remove() : false,
        ...
        this.talkingCallback = scene.time.delayedCall(
          ...
          () => {
            //==不同事件的處理方法
            const platform = gameScene.platforms.getChildren()[0], //==物品出現在地上
            ...
            switch (eventIdx) {
              case 0: //===躲避地震
                eventAction = () => {
                  //==SFX
                  gameScene.sound.play("sfx_bossAttack");
                ...
                //===防災物品
                getItems = () => {
                  let table = scene.physics.add
                    .sprite(width * 0.5, floorY, "emergTable")
                    .setName("emergTable")
                    .setScale(0.2)
                    .setOrigin(0.5, 0.9)
                    .setAlpha(0);
                  ...
                  hintTexts.goalText = goals.map((goal, i) => {
                    ...
                    let hint = scene.add
                      .text(width * 0.8, this.ruleText.y + 50 * i, "", {
                        fontSize: "24px",
                        fill: "#fff",
                        stroke: "#000",
                        strokeThickness: 1,
                      ...
                      .setDepth(1);
                    let sign = scene.add
                      .image(hint.x, hint.y, "emergWrong")
                      .setAlpha(0)
                      .setOrigin(1, -2)
                      .setScale(0.1)
                      .setDepth(1);
                  ...
                  hintTexts.goalText.forEach((sign, i) => {
                    ...
                    switch (i) {
                      ...
                      case 1:
                        ...
                        condition =
                          Math.abs(player.x - table.x) < table.displayWidth / 2;
                        break;
                      case 2:
                        condition =
                          hintTexts.completion[0] && hintTexts.completion[1];
                        break;
                    }
              ...
              case 1: //===撿防災物品
                ...
                getItems = () => {
                  ...
                  const getHint = (item) => {
                    let string =
                      item.name === "emergBag"
                        ? `${itemHints[item.name]} 0 / ${
                            emergencyKitItems.length
                          }`
                        : itemHints[item.name];

                    let hint = scene.add
                      .text(item.x, item.y - item.displayHeight, string, {
                        fontSize: "24px",
                        fill: "#fff",
                        stroke: "#000",
                        strokeThickness: 1,
                  ...
                  //==防災包
                  let emergBag = scene.physics.add
                    .sprite(width * 0.5, floorY, "emergBag")
                    .setName("emergBag")
                    .setScale(0.8)
                    .setOrigin(0.5, 0.9)
                    .setAlpha(0);
                  ...
                  let list = [...emergencyKitItems].sort(
                    () => 0.5 - Math.random()
                  ); //==拷貝陣列
                  let amount = emergencyKitItems.length; //=物品數量
                  [...Array(amount).keys()].forEach((i) => {
                    let itemKey = list.pop(),
                      itemX =
                        (width / (amount + 3)) * (i + 1 + (i > 2 ? 2 : 0));

                    let item = scene.physics.add
                      .sprite(itemX, floorY, itemKey)
                      .setName(itemKey)
                      .setScale(0.4)
                      .setOrigin(0.5, 0.9)
                      .setAlpha(0);
                        ...
                        if (!item.beholding) {
                          ...
                          scene.tweens.add({
                            ...
                            onComplete: () => {
                              item.destroy();
                              emergBag.itemCount++;
                              let text = `${itemHints[emergBag.name]} ${
                                emergBag.itemCount
                              } / ${amount}`;
            ...
            //==事件發生
            let startCall = scene.time.delayedCall(
              eventDuration * 0.8,
              () => eventAction(),
              [],
              scene
            ...
            //==事件結束
            let endCall = scene.time.delayedCall(
              eventDuration,
              () => this.emit("emergEnd"),
              [],
              scene
            ...
            scene.time.delayedCall(100, () => {
              //==敵人暫停
              gameScene.enemy.children.iterate(
                (child) => (child.behavior = "worship")
              );
            ...
            scene.tweens.add({
              ...
              onStart: async () => {
                ...
                for (let i = 0; i < rulesCount; i++) {
                  ...
                  let lineType = i === 0 ? 0 : i === rulesCount - 1 ? 2 : 1;
...
class Chunk {
  constructor(scene, x, y) {
    ...
    scene.physics.add.collider(
      ...
      this.tiles,
      this.gameScene.player.playerDig,
      null,
      this
    );
    ...
    scene.physics.add.overlap(
      this.gameScene.player,
      this.liquid,
      this.gameScene.player.playerSwim,
      null,
      this
    );
    this.isLoaded = false;
    ...
    let getYRange = (km) =>
      scene.groundY + parseInt(km / scene.depthCounter.depthScale);
    ...
    if (!this.isLoaded) {
      ...
      for (let y = 0; y < this.gameScene.chunkSize; y++) {
        ...
        for (let x = 0; x < this.gameScene.chunkSize; x++) {
          ...
          let ECtileCount = Math.ceil(
            depthCounter.epicenter / depthCounter.depthScale / tileSize
          );
          let tileYRange = [
          ...
          if (
            ...
            let bossX =
                Math.ceil(tileXRange.reduce((p, c) => (c + p) / 2) / tileSize) *
                tileSize,
              bossY = tileYRange[1] - tileSize * 2;

            //門
            if (tileX == bossX && tileY == bossY) {
              let bossCastle = this.gameScene.physics.add.sprite(
                bossX,
                bossY + tileSize * 1.1,
                0,
                0,
            ...
            //門下一排石塊
              ...
              //門下無法破壞石塊
              key =
                tileX >= bossX - 2 * tileSize && tileX < bossX + 2 * tileSize
                  ? "gateStone"
                  : "";
            }
            //火把
            else if (
              (tileX == bossX - 2 * tileSize ||
                tileX == bossX + 2 * tileSize) &&
              tileY == bossY
            ) {
              let bossTorch = this.gameScene.add.sprite(
                tileX,
                tileY,
                "bossTorch"
              );
...
class RexTextBox extends RexPlugins.UI.TextBox {
  constructor(scene, config, resolve = null) {
    ...
    let getTipColor = (isBox = true) => {
      //==每個助手對話框不同色
      let name =
        character == "sidekick" ? scene.gameData.sidekick.type : character;
      // console.debug(name);
    ...
    const COLOR_PRIMARY = !tips ? 0x4e342e : getTipColor(true), //==box背景色
    ...
    //===textBox config
    let roundCorner = tips ? (character == "doctor" ? 10 : 40) : 20;
    let rexRect = new RexPlugins.UI.RoundRectangle(
      scene,
      0,
      0,
      2,
    ...
    if (!tips) {
      ...
      let BgColor = isPlayer
        ? gameData.playerCustom.avatarBgColor
        : character == "doctor"
        ? 0x00ec00
        : undefined;
      let img = new Phaser.GameObjects.Image(
        scene,
        0,
        0,
      ...
      img.setScale(imgW / Math.max(img.width, img.height)).setDepth(3);

      icon = new RexPlugins.UI.Label(scene, {
        background: scene.add.existing(
          new RexPlugins.UI.RoundRectangle(scene, 0, 0, 0, 0, 5, BgColor)
      ...
      //==跳過...等
      let dialogButton = scene.add
        .image(0, 0, "dialogButton")
        .setVisible(false)
        .setScale(0.1)
      ...
      let skip = scene.add
        .text(0, 0, gameData.localeJSON.UI["skip"], {
      ...
      let prevLine = scene.add
        .text(0, 0, gameData.localeJSON.UI["prevLine"], {
          font: "20px sans-serif",
          fill: "#7B7B7B",
        })
        .setOrigin(0.5, 1)
    ...
    if (!tips) {
      ...
      keyObj.on("down", () =>
        !scene.scene.isPaused() ? this.emit("pointerdown") : false
      );
      ...
      skip
        ...
        .on("pointerover", function () {
          this.setScale(1.5).setColor("#FFFF37");
        })
        .on("pointerout", function () {
          this.setScale(1).setColor("#7B7B7B");
        })
        .on("pointerdown", () => {
          this.destroy();
          resolve(0);
...
class RexDialog extends RexPlugins.UI.Dialog {
  constructor(scene, config, resolve) {
    ...
    const COLOR_PRIMARY = !quizType ? 0x333333 : 0x003c8f, //==box背景色
    ...
    let setDialog = (data) => {
      ...
      if (!quizType) {
        ...
        // Set choices
        let choiceTextArray = GetValue(data, "choices", []).sort(
          () => 0.5 - Math.random()
        );
        let choiceText;
      ...
      .on(
        ...
        (button, groupName, index) => {
          ...
          if (!quizType) {
            ...
            let sign = scene.add
              .image(
                button.x + button.displayWidth * 0.5,
                button.y,
                "quiz" + (answer ? "Correct" : "Wrong")
              )
              .setScale(0.1)
    ...
    if (quizType == 2) {
      ...
      //==監聽鍵盤按鍵
      let clearKeyName = (key) =>
        config.localeJSON[key] ? config.localeJSON[key] : key;
      scene.input.keyboard.on("keyup", (e) => {
        let key = Object.keys(keyCode).find(
            (key) => keyCode[key] === e.keyCode
          ),
          isKeyExisted = Object.values(config.tmpData).indexOf(key) !== -1;

        if (isKeyExisted) {
          content.setText(
            `[size=40][color=red][b]${clearKeyName(key)}[/b][/color][/size]\n${
              config.localeJSON["keyRepeat"]
            }`
          );
        } else {
          content.setText(
            `[size=40][color=green][b]${clearKeyName(key)}[/b][/color][/size]`
          );
    ...
    switch (panelType) {
      ...
      case 1:
        ...
        data = {
          name: panelName,
          category: {
            control: Object.keys(tmp.controllCursor).map(
              (key) => new Object({ name: key })
            ),
          },
        };
        if (!config.noLocleSetting)
          data.category.language = [{ name: tmp.locale }];
    ...
    let createPanel = (scene, data) => {
      ...
      let createTable = (scene, key = null) => {
        ...
        let getText = (text, major = true) => {
          ...
          if (!major)
            BBCodeText.setInteractive().on("areadown", function (key) {
        ...
        let table = new RexPlugins.UI.Sizer(scene, {
          orientation:
            panelType === 1 || key === "hotkey" ? scrollMode : !scrollMode,
        ...
        if (key) {
          ...
          let grid = new RexPlugins.UI.GridSizer(scene, {
            ...
            createCellContainerCallback: (scene, col, row, config) => {
              ...
              switch (key) {
                case "control":
                  let clearKeyName = (key) =>
                    localeJSON.UI[key] ? localeJSON.UI[key] : key;
                  ...
                  Panel.on("resetControl", () => {
                    ...
                    chidren[1].getElement("items").forEach((grid, i) => {
                      if (grid) {
                        ...
                        let keyName =
                          tmp.controllCursor[data.category[key][i].name];
                ...
                case "language":
                  ...
                  keyPressAction = async (icon, keyPressed) => {
                    ...
                    if (newData !== tmp.locale) {
                      ...
                      // console.debug(this);
                      this.getElement("header").setText(
                        localeJSON.UI[data.name]
                      );
                      ...
                      Object.keys(data.category).forEach((key, i) => {
                        ...
                        chidren[1].getElement("items").forEach((grid, i) => {
                          if (grid) {
                            ...
                            if (key == "control")
                              child[1].setText(
                                localeJSON.UI[data.category[key][i].name]
                              );
                  ...
                  Panel.on("resetLanguage", () =>
                    keyPressAction(icon, 0, true)
                  );
                  break;
                default:
                  itemText = false;
                ...
                .on("pointerdown", async function () {
                  let keyPressed = await new Promise((resolve) => {
                    ...
                    Object.keys(questionData).forEach((key) =>
                      key == "question"
                        ? (newQuestionData[key] =
                            tmp.localeJSON.UI[questionData[key]])
                        : (newQuestionData[key] = questionData[key].map(
                            (k) => tmp.localeJSON.UI[k]
                          ))
                    );
                    // console.debug(questionData)
                    let confirmScene = scene.scene.add(
                      null,
              ...
              if (categoryType === 0)
                gridItem.add(
                  scene.add.text(0, 0, localeJSON.UI[item.name], {
                    padding: padding,
                  }), // child
                  { padding: { left: 10 } }
                );
          ...
          switch (panelType) {
            case 0: //==緣由
              const introImgs = ["epicenter", "PSwave"], //震央圖,PS波圖
                introLinks = [
              ...
              break;
            case 2: //==連結
              const sites = ["GDMS", "BATS", "TECDC", "READYTW"],
                siteLinks = [
            ...
            case 3: //==排行
              ...
              let getRankData = (rankType = 0) => {
                ...
                let newData = rankingData.sort((a, b) =>
                  rankType ? b[rankCol] - a[rankCol] : a[rankCol] - b[rankCol]
                );
                newData =
                  newData.length > rankAmount
                    ? newData.slice(0, rankAmount)
                    : newData;
                ...
                newData = newData.map((d) => {
                  ...
                  if (rankType)
                    ...
                    let timeUse = {
                      ...
                      timeUseStr =
                        (timeUse.hour > 0
                          ? timeUse.hour + localeJSON.UI["HRS"]
                          : "") +
                        (timeUse.hour > 0 || timeUse.min > 0
                          ? timeUse.min + localeJSON.UI["MINS"]
                          : "") +
                        timeUse.sec +
                        localeJSON.UI["SECS"];
              ...
              class rankingBoard extends RexPlugins.UI.GridSizer {
                constructor(rankingData) {
                  ...
                  const rankColumns = colKeys.length,
                    rankRows = rankingData.length;

                  const part = [1, 2, 2],
                    eachWidth =
                      (config.width * 0.8) / part.reduce((p, c) => p + c),
                  ...
                  let RKconfig = {
                    ...
                    createCellContainerCallback: function (
                      ...
                      let text =
                        row === 0
                          ? localeJSON.UI[rankingData[row][col]]
                          : col === 0
                          ? row
                          : rankingData[row][colKeys[col]];
                          ...
                          row === 0
                            ...
                            : scene.add.existing(
                                new RexPlugins.UI.RoundRectangle(
                                  ...
                                  0,
                                  0,
                                  row % 2 ? boardColor[0] : boardColor[1]
                                )
                              ),
                        text: scene.add
                          .text(0, 0, text, {
    ...
    let createFooter = (scene, footerItem) => {
      let createButton = (text) => {
        return new RexPlugins.UI.Label(scene, {
          ...
          text: scene.add
            .text(0, 0, gameData.localeJSON.UI[text], {
              fontSize: 18,
              padding: padding,
            })
            .setOrigin(0.5),
...
class RexForm extends RexPlugins.UI.Sizer {
  constructor(scene, config, resolve) {
          ...
          if (ok) {
            let localeJSON = gameData.localeJSON,
              questionData =
                playerName != ""
                  ? {
                      question: localeJSON.UI["avatarConfirm"].replace(
                        "\t",
                        ` "${playerName}" `
                      ),
    ...
    let createID = (scene) => {
      ...
      let sidekickBox = new RexPlugins.UI.Sizer(scene, { orientation: 0 }).add(
        label(gameData.localeJSON.UI["chooseSidekick"])
      );
      config.sidekicks.forEach((sidekick, i) => {
        let icon = sideKickIcon(sidekick);
        if (i === 0)
          icon.getElement("background").setStrokeStyle(5, COLOR_LIGHT);
    ...
    let createGrid = (scene, isTexture = false) => {
      let createCell = (index) => {
        ...
        let key = isTexture ? index : Phaser.Math.Between(0, 0x1000000);
        ...
        return new RexPlugins.UI.Label(scene, {
          background: scene.add.existing(
            new RexPlugins.UI.RoundRectangle(
              ...
              0,
              5,
              isTexture ? undefined : key
            )
          ),
          icon: isTexture
            ? scene.add.image(0, 0, character + "_avatar" + key)
            : false,
          name: key,
          space: {
            left: 10,
          ...
          .on("pointerdown", function () {
            ...
            if (isTexture) {
              avatarSelect
                .getElement("icon")
                .setTexture(character + "_avatar" + name);
              avatarIndex = name;
            } else {
    ...
    this.addBackground(background)
      .add(
        createHeader(
          scene,
          gameData.localeJSON.UI["characterSet"],
          COLOR_LIGHT
        ),
        {
          proportion: 0,
      ...
      .add(
        scene.add.text(0, 0, gameData.localeJSON.UI["choosePhoto"], {
          fontSize: "24px",
          padding: padding,
        }),
        {
      ...
      .add(
        scene.add.text(0, 0, gameData.localeJSON.UI["chooseBGcolor"], {
          fontSize: "24px",
          padding: padding,
        }),
        {
    ...
    this.setScale((config.sceneHeight * 0.9) / this.height).setPosition(
      config.sceneWidth - this.displayWidth * 1.1,
      config.sceneHeight * 0.5
    );
  }
}
...
class RexSheet extends RexPlugins.UI.FixWidthSizer {
  constructor(scene, config, resolve) {
      ...
      .on("postlayout", function (children) {
        children
          .filter(
            (child) =>
              child.type === "Text" ||
              (child.type === "Image" && child.name === "pic")
          )
          .forEach((child) => {
            let duration, delay;
    ...
    let arrowAnime = (arrowIdx, show = true) => {
      ...
      if (show) {
        ...
        let arrowPos =
          arrowIdx === 0
            ? [
                pic.x - pic.displayWidth * 0.27,
                pic.y + pic.displayHeight * 0.18,
              ]
            : [
                pic.x - pic.displayWidth * 0.09,
                pic.y - pic.displayHeight * 0.4,
              ];
        ...
        let tweens1 = scene.tweens.add({
          ...
          onStart: (tween, targets) => {
            ...
            hintGroup[2]
              .setPosition(
                arrowPos[0] + 46,
                pic.y - pic.displayHeight * 0.5 - 10
              )
    ...
    let getText = (str) => {
      ...
      let text = scene.add
        .text(0, 0, str, {
          ...
          padding: padding,
          color:
            keyWordIdx >= 0 ? ["#005AB5", "#750000"][keyWordIdx] : "#000000",
        })
        .setOrigin(0.5);
        ...
        text
          ...
          .on("pointerover", () => {
            ...
            keyWords.forEach((key, i) =>
              arrowTweens[i] && arrowTweens[i].length !== 0
                ? arrowAnime(i, false)
                : false
            );
    ...
    let spitKey = (words) => {
      ...
      let IdxOfArray = keyWords.map((key, i) =>
        words[words.length - 1].indexOf(key)
      );
    ...
    this.add(img, { key: "pic" }).addNewLine();
      ...
      return new RexPlugins.UI.Label(scene, {
        background: scene.add.existing(
          new RexPlugins.UI.RoundRectangle(
            ...
            40,
            5,
            i % 2 === 0 ? COLOR_PRIMARY : COLOR_SECONDARY
    ...
    scene.time.delayedCall(
      500,
      () => keyWords.forEach((key, i) => arrowAnime(i, true)),
      [],
      scene
    );
  }
      ...
      switch (type) {
        case "jump":
          animObj = {
            ...
            onProgress: function (char, t) {
              ...
              let p1 = p0 - 20;
              let value = Phaser.Math.Linear(
                p0,
                p1,
                Phaser.Math.Easing.Cubic.Out(t)
              );
              char.setY(value);
            },
          ...
          animObj = {
            ...
            onProgress: function (char, t) {
              ...
              if (t < percent) {
                ...
                let x = Phaser.Math.Linear(
                  char.x,
                  pos[0],
                  Phaser.Math.Easing.Circular.InOut(p)
                );
                char.setX(x);
              } else {
                ...
                let y = Phaser.Math.Linear(
                  pos[1],
                  pos[1] + config.height / 2,
                  Phaser.Math.Easing.Circular.In(p)
                );
                char.setAlpha(1 - p).setY(y);
              }
            },
        ...
        case "fallout":
          animObj = {
            ...
            onStart: function (char) {
              // console.debug(char);
              char.setData("pos", [char.x, char.y]).setY(0).setVisible();
            },
            onProgress: function (char, t) {
              let percent = 0.9; //==往左佔時間比例 (t-t0)/percent
              let pos = char.getData("pos");
              if (t < percent) {
                ...
                let y = Phaser.Math.Linear(
                  char.y,
                  pos[1],
                  Phaser.Math.Easing.Cubic.Out(p)
                );
                char.setY(y);
              } else {
                let p = getLinearP(t, percent, 1 - percent); //==0.8-1要換算成0-1
          ...
          animObj = {
            ...
            onProgress: function (char, t) {
              ...
              if (t < percent) {
                ...
                let rotation = Phaser.Math.Linear(
                  0,
                  -Math.PI * 2,
                  Phaser.Math.Easing.Expo.Out(p)
                );

                char.setRotation(rotation).setY(y);
                // console.debug(char);
              } else {
                let p = getLinearP(t, percent, 1 - percent); //==0.8-1要換算成0-1
                let scale2 = p < 0.3 ? 1.5 : 0;
                let scale = Phaser.Math.Linear(
                  char.scaleX,
                  scale2,
                  Phaser.Math.Easing.Circular.InOut(p)
                );
                char.setAlpha(1 - p).setScale(scale);
              }
            },
            onComplete: function (char, t) {},
          };
          ...
          animObj = {
            ...
            onProgress: function (char, t) {
              ...
              let x = Phaser.Math.Linear(
                char.x,
                pos[0],
                Phaser.Math.Easing.Circular.InOut(t)
              );
              char.setX(x);
            },
            onComplete: function (char, t) {},
